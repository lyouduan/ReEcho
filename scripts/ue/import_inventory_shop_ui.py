...
SOURCE_ROOT = Path(unreal.Paths.project_content_dir()) / "SourceArt" / "UI"
...
for source_name, asset_name in IMPORTS.items():
    ...
    if not source.is_file():
        raise RuntimeError(f"Missing UI source: {source}")
    ...
    if len(task.imported_object_paths) != 1:
        raise RuntimeError(
            f"Expected one imported texture for {source_name}, got: {task.imported_object_paths}"
        )
    ...
    unreal.log(f"Imported UI background: {task.imported_object_paths[0]}")
