"""Make the Fox windup direction arrow follow its attack root and locked direction."""
...
if not unreal.ReEchoCombatVfxComponent.set_niagara_system_emitters_local_space(system):
    raise RuntimeError(f"Failed to set enabled emitters to local space: {SYSTEM_PATH}")
...
unreal.log(f"FOX_DIRECTION_LOCAL_SPACE_OK system={SYSTEM_PATH}")
