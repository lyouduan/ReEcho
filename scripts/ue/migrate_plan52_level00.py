"""Replace Level00's Arena instance with BP_ArenaScene_SC01 while preserving authored settings."""

import unreal


LEVEL_PATH = "/Game/Level00"
TARGET_CLASS_PATH = "/Game/ReEcho/Scene/Prefabs/BP_ArenaScene_SC01.BP_ArenaScene_SC01_C"
PROFILE_PATH = "/Game/ReEcho/Scene/Profiles/DA_ArenaScene_SC01"
ACTOR_PROPERTIES = (
...
if arena_class is None or target_class is None or target_profile is None:
    fail("Required native class, target Blueprint class, or SC01 profile is unavailable")
...
if old_actor.get_class() == target_class:
    ...
    if not level_subsystem.save_current_level():
        fail("Could not save already-migrated Level00")
    unreal.log("[Plan52] Level00 already uses BP_ArenaScene_SC01; refreshed its profile")
else:
    ...
    if new_actor is None:
        fail("Could not spawn BP_ArenaScene_SC01")
    ...
    if new_actor.get_editor_property("scene_profile") != target_profile:
        fail("New Arena Scene did not retain the SC01 profile")
    ...
    if len(remaining) != 1 or remaining[0] != new_actor:
        fail("Level00 Arena Scene uniqueness check failed after migration")
    if not level_subsystem.save_current_level():
        fail(f"Could not save {LEVEL_PATH}")
    unreal.log("[Plan52] Migrated Level00 to BP_ArenaScene_SC01 with preserved authored settings")
