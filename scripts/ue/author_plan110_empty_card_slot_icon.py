...
if not isinstance(frame, unreal.Texture2D) or not isinstance(lock, unreal.Texture2D):
    raise RuntimeError("Missing reviewed card-slot frame or lock texture")
...
for index in range(3):
    ...
    offer_art.set_brush_from_texture(lock, False)
...
unreal.log(
    f"[Plan110EmptySlotAuthor] widget={WIDGET_PATH} cleared_loadout_slots=12 pack_offers=3 frame={FRAME_PATH} lock={LOCK_PATH}"
)
