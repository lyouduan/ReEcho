...
美术 AI 提交遵循 `shared/GIT_RULES.md` 的身份规则；可按 `shared/PROJECT_RULES.md` 推送 `artist/<task>` 远端协作分支，但这不授予 `main` 发布权限。
...
- 需要跨机器保存、评审或交接时可按 `PROJECT_RULES.md` 非强制推送 `artist/<task>`；不得直接推送或发布 `main`，最终提交/资产和证据仍交给程序 Planner 集成。
