...
- `origin/main` 是唯一权威发布分支，只有进入它的内容才构成仓库正式规则、Plan、源码、数据或资产状态。程序路线与项目秘书默认仍只推送 `origin/main`，不得创建或推送其他远端分支；例外按本文件风险确认机制处理。
...
- 策划/美术推送协作分支前必须先 fetch `origin/main` 及目标远端分支，审计同路径物理冲突、逻辑冲突和耦合；提交身份遵循 `GIT_RULES.md`。禁止强推、覆盖他人远端历史、删除他人分支或把程序实现混入专业分支；这些操作仍按本文件风险确认机制处理。
