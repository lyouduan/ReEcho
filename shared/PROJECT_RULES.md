...
- 风险操作包括但不限于：覆盖或丢弃未提交内容、删除脏 worktree、强制删除分支/worktree、reset/rebase/重写历史、强推或删除远端引用、创建默认策略之外的远端分支、越过活跃所有权或职责边界、手改生成/二进制工件、扩大范围，以及跳过规则或 Plan 要求的构建、测试、审查或发布门禁。
...
- Executor 默认不修改、合并或推送 `main`，也不推送任何远端引用。Planner 可创建已评审本地合并提交，并依据 `PLANNER_RULES.md` 发布远端 `main`；其他职责或操作需按本文件准确确认。
...
- 默认仅显式暂存路径，不使用 `git add .` / `git add -A` 或强推 `main`；确需执行时先列出会纳入的路径或被改写的远端历史，建议问程序并取得准确人工确认。
