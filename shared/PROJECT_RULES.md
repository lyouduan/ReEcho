...
- 策划或美术 AI 可自由组织本地分支、worktree 和提交，记录准确资产/数据变化，并将结果交给程序路线集成；不得自行推送任何远端引用或发布 `main`。
...
- 每个成员、AI 和克隆的本地工作自治：可自行选择直接工作、分支、worktree、stash、WIP 提交、merge 或 rebase。分支和 worktree 是隔离建议，不是共享权限门禁；本地命名、提交粒度和并行安排不需要写入共享实时状态。
- 本地自治不扩大产品、专业角色或破坏性操作权限。覆盖他人未提交内容、删除脏/未合并工作、重写历史等风险操作仍遵循本文件的人类确认机制。
- 默认情况下，`origin/main` 是唯一允许的远端分支，不推送任何远端引用形式的规划、任务、交接、评审或 PR 分支；例外按本文件风险确认机制处理。
...
- 远端状态可能变化时，唯一允许自动执行的第一步是 `git fetch`。若发现当前已批准本地基线之外的他人提交，执行 AI 必须在 pull、merge、rebase、cherry-pick 或 push 前停止；报告物理/Git 冲突、逻辑冲突和集成耦合，然后等待用户明确选择。可快进或干净自动合并都不例外。
- 若没有新的他人提交，完成适用门禁后可继续，不需要额外询问。默认工作模式是：**能合并就合并、能推送就推送、能删除就删除**；这里的“能”表示范围、验证、人工验收、远端审计、发布和清理安全门禁均已满足。主观验收、产品取舍、外部提交语义选择和可能丢失工作的清理必须提醒并等待人决定。
- 鼓励小批量、较频繁地发布 main，以缩短跨机器差异窗口；不得用高频发布绕过 Plan、外部提交审计、人工验收或程序最终构建。
- 默认仅显式暂存路径，不使用 `git add .` / `git add -A` 或强推 `main`；确需执行时先列出准确影响并按本文件取得确认。
