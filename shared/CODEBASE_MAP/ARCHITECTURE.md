...
Plan41 的四模块拓扑和 Plan43 的第五个 Runtime Module `ReEchoEnemies` 均已进入 `main`。Plan44 在既有单向依赖上增加配表驱动的 Boss 技能策略、EnemyHost 世界执行、Combat 清洗窄命令和 v8 Boss 保存恢复。Plan47 `Review` 候选新增第六个 Runtime Module `ReEchoCards`、39 张新构筑和 v9 CardState 保存链；Cards 只依赖 Combat，Weapons/Enemies 不反向依赖 Cards，主模块负责所有世界与商店适配。
