...
永久最大生命变化及其当前生命语义通过 `ApplyHealthAdjustment(NewMaximumHealth, NewCurrentHealth, Adjustment)` 这一窄命令进入 Combatant；`SetToStatPoint` 精确采用提交的当前生命，`FillToMax` 忽略该值并回满。它在一次调用内更新 GAS 基础属性（无 ASC 时更新兼容状态），在批量写入期间抑制中间态，提交后统一发布
