...
Plan76 的晕眩、流血、短暂无敌和临时攻速/移速均通过 `UReEchoCombatantComponent` 的窄命令进入 Combat。`Z_Vertigo` 维护动作禁止边界；`Z_Bleeding` 每层每秒结算最大生命 0.5%，每层独立保存到期时间；临时属性层通过独立 GAS Effect Handle 应用和移除。攻速参数是相对中性值 `1.0` 的加法百分点，每个 `+1%` 层精确增加 `0.01`，与基础值大小无关；移速继续按未修改基础值的乘数百分比计算。非 GAS fallback 必须与两种 GAS 运算分别一致并在移除时精确恢复。Weapons/主模块只能提交命令并读取查询，不能直接改生命、状态标签或最终属性。
