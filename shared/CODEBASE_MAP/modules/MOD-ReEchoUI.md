...
- Plan132 把开场 Loadout 从角色/武器同页一次确认改为同一 `WBP_ReEchoLoadoutSelection` 内的两个连续阻塞阶段。Widget 只拥有当前阶段、Preview 索引和未提交的两个稳定 ID：角色确认只切换到武器阶段，武器确认才沿既有 `OnLoadoutConfirmed(CharacterId, WeaponId)` 向 GameMode 广播一次最终组合；Run、预加载和首关切换权威不变。角色/武器资格与说明仍来自 CSV 快照，交付构图顺序仅是稳定 ID 的页面表现排序。`WBP_ReEchoLoadoutEntry` 用成对 Selected/Unselected Texture2D 表达“初始全亮 / Preview 当前亮其余暗”；鼠标 Hover 复用商店的原生 Tooltip 定位机制，跟随当前条目并由 Slate 做屏幕避让。`WBP_ReEchoLoadoutTooltip` 拥有鼠标说明框的宽度、边框、内边距、字体、字号、颜色和换行，C++ 只写入 CSV 标题/说明；键盘/手柄 Focus 才显示页面内大号锚定回退框，避免同一 Hover 同时出现两块说明。整屏合成稿不作为交互贴图。正式纹理目录由 Packaging AlwaysCook 收集，单项缺图只回退既有角色/武器纹理，不改变资格或事务。
