...
- 同一商店页展示数据驱动的武器槽组、兼容符文报价和装配草稿；商品购买按钮一律发送购买事务，不再兼做已拥有物品的免费重装；I 级符文允许跨刷新页重复购买。正式背包装配走独立的 `OnOwnedPartEquipRequested` 命令，备用草稿页的“保存配置”仍发送完整 PartId 集合给 Run。购买不直接装备，只有与当前装备合成时由 Run 在原槽升级。符文背包在 Run 投影基础上仍按当前武器类型与所点槽位做防御性过滤，长剑和镰刀共享槽名时不展示对方专属符文；关闭页面不提交草稿，已成功购买的符文所有权仍保留。
