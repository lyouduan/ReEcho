...
- 商店逻辑按稳定的无资产区块拆分：`ShopLogicScrollBox > ShopLogicPanel` 承载 `RunItemOfferPanel`（普通商品）、`ShopControlPanel`（规则/刷新）、`WeaponPartOfferPanel`（配件购买）和 `WeaponLoadoutPanel`（槽位草稿/保存）；`EchoPanel` 使用独立的底部缩放托盘与显式高 ZOrder。这些名称是后续 WBP 接入的逻辑契约，C++ 不依赖任何美术占位节点。
