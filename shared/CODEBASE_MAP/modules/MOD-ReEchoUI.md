...
- Plan118 将 Minimap Canvas 的 Echo 轨迹从 2px 纯色折线改为沿同一投影路径确定性盖印经典墨水笔尖。`UReEchoMinimapCanvasWidget` 硬引用 UI 材质以进入 Cook，Slate 绘制继续乘入每个 Echo 的既有颜色；笔尖尺寸、间距、角度/透明度抖动和 Grain 强度在该控件实例的 `Minimap | Ink Trail` 分类调整。盖印跨折线段保持间距，并受单 Echo 最大数量约束；材质缺失时才回退旧折线，不改变录制采样、路径数据、头像或坐标投影。
