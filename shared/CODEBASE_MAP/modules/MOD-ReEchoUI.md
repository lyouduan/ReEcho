...
- Plan132 把开场 Loadout 从角色/武器同页一次确认改为同一 `WBP_ReEchoLoadoutSelection` 内的两个连续阻塞阶段。Widget 只拥有当前阶段、Preview 索引和未提交的两个稳定 ID：角色确认只切换到武器阶段，武器确认才沿既有 `OnLoadoutConfirmed(CharacterId, WeaponId)` 向 GameMode 广播一次最终组合；Run、预加载和首关切换权威不变。角色/武器资格与说明仍来自 CSV 快照，交付构图顺序仅是稳定 ID 的页面表现排序。`WBP_ReEchoLoadoutEntry` 用成对 Selected/Unselected Texture2D 表达“初始全亮 / Preview 当前亮其余暗”，Hover、Focus 和点击统一驱动 Preview；整屏合成稿不作为交互贴图。正式纹理目录由 Packaging AlwaysCook 收集，单项缺图只回退既有角色/武器纹理，不改变资格或事务。
