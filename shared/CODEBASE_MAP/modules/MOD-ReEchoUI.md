...
- 世界空间伤害跳字由 `AReEchoDamageNumberActor` 硬引用 `/Game/ReEcho/Fonts/DamageNumbers/F_DamageNumber_MFYuYue_Font`，保证 Cook 收集；对应 OTF 与授权说明归档在 `Content/SourceArt/UI/CombatHud/DamageNumbers/`，只允许用于非商用伤害数字，不得当作全局 UI 字体复用。Enemy Presentation 只读 `FReEchoDamageEvent::ReactionBehaviorId` 选择反应色：Vaporize `#A5CBF3`、Conduct `#EBC02C`、Burn `#E86A12`、Growth `#92C039`、Enhance `#F1B84C`；无反应标识时回退到既有元素色/物理白色，不重新推导反应。Actor 使用专用 `M_DamageNumberTextTranslucent`：完整保留 UE 默认文字材质的距离场字形重建，只在其最终 Opacity 后乘 TextRender 顶点 Alpha；整个 `0.9s` 上漂生命周期内将初始颜色 Alpha 线性乘到零，确保字体外观不变并在画面中逐渐透明后再销毁。`scripts/ue/author_damage_number_material.py` 可幂等重建该材质。
