...
- 相关 Plan：Plan29、Plan34、Plan45、Plan51、Plan70、Plan93、Plan99、Plan102、Plan110、Plan116、Plan118、Plan132、Plan145、Plan147、Plan149、Plan150、Plan154、Plan162。
...
- 商店装配树的卡牌槽框由 `DesignerCardSlot0..11` 的 Button 四态持有，子图层 `DesignerCardSlotArt0..11` 只承载已拥有卡牌内容；装配树固定提供两页，第一页投影第 1–12 张、第二页投影第 13–24 张，第二页继续复用同一组作者化槽位及其位置，不复制第二套几何或卡牌状态。`DesignerPageCounter` 展示 `1/2`、`2/2`，`ArtFormalPageLeft` / `ArtFormalPageRight` 负责切页；透明命中层只接收点击，箭头图本身按统一按钮契约在可用时以中心 `1.05` 倍悬停缩放、移出恢复作者尺寸。左侧可打开候选的 `DesignerPackOfferIcon0..2` 卡组商品位（`Available` / `PaidPendingChoice` 且确有候选）使用 `T_UI_Shop110_CardPackOfferIcon` 卡叠图；`NotOffered`、已结束或没有候选的状态继续使用 `T_UI_Shop110_EmptyCardSlotIcon` 锁图。是否存在固定Tier条目本身不代表已经投放。
...
- 难度选择由 `WBP_ReEchoSettings` 顶部与“画面 / 声音 / 键位”并列的作者化“难度”第四页签进入；独立 `DifficultyPanel` 内纵向保存“派对 / 常规 / 噩梦”三个暂停按钮风格控件。页签、页面、位置、尺寸、默认文字和视觉层均可在 Designer 中直接预览及调整。`UReEchoSettingsWidget` 只切换分类显隐、选中图、可用状态并把未开局时的选择提交给 `UReEchoRunSubsystem`，不得创建下拉框或覆盖作者几何。StartRun 或 Continue 锁定快照后，同一页面仅显示本局值并禁用三个按钮；Widget 不加载 CSV、不决定回退、不持有第二份本局难度。恢复默认只把下一局偏好设为“常规”。
...
| 修改难度选择/锁定展示 | `ReEchoSettingsWidget.*` | `UReEchoRunSubsystem` 偏好/本局窄接口、`MOD-ReEcho.md/AREA-Data/Run` |
