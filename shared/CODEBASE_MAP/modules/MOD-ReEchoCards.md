...
敌人基础碎片数值不属于 Cards。Cards 只通过 `FReEchoCardRuleSnapshot::bDisableEnemyShardDrops` 和 `BonusShardDropEncounterIndex` 声明禁掉落/下一场逐只 1.5 倍修正；`UReEchoRunSubsystem::GrantEnemyDeathTimeShards` 是唯一消费端，关末只清理一次性标记，不再固定发放货币。
