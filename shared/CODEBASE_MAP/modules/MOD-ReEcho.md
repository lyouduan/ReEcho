...
| Encounter 结算表现状态 | `AReEchoGameMode` | 单场结束到局间 UI | 只投影 Director 剩余时间；普通计时关卡在 03→01 时只启动全屏后处理，权威 00 时冻结战斗并从第 0 帧播放透明序列，末帧后幂等进入抽卡；第2–7关全部免费抽卡成功耗尽后，再播放独立透明序列并在覆盖层下切换到商店 |
