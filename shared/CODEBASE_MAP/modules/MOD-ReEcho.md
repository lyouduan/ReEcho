...
- 近战刀光范围适配（Plan148）：WeaponActor 在一次 Commit 上统一应用静态 AttackStep 与临时范围层，并把最终范围、基础范围、倍率和角度随 `AttackCommitted` 快照。长剑和镰刀刀光仍从 `WeaponAttackVfxRoot` 生成，仅其 Niagara Component 的 DA 声明局部径向轴消费范围倍率；根节点、武器尺寸、Slot Offset、左右播放和玩法命中不参与缩放。
- 镰刀 FullSpin（Plan148）保留 WeaponActor 的相机平面旋转，但用手部挂点、当前镜像后 `HeldOffsetRatio` 与旋转四元数反向补偿 Actor Location，使镰刀可见组件中心在整圈动作中保持不动；长剑 TripleSwing 继续以手部挂点为枢轴，不共享该补偿。
