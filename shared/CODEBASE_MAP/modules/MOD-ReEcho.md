...
- 战斗 VFX：详细设计见 [`MOD-ReEchoVFX.md`](MOD-ReEchoVFX.md)。`UReEchoCombatVfxComponent` 装配在 Player、Echo 与 Enemy Host，只读订阅 Combat/Enemy 类型化事件；`FReEchoCombatVfxCatalog` 是语义到完整 Niagara 路径的唯一映射。兔子可见子弹跟随 EnemyHost 的逻辑投射物快照，Niagara 不拥有轨迹、碰撞、伤害或存档。
