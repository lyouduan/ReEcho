...
- 战后卡牌投放：Run 按当前 `EncounterIndex` 查询 `shop_drop_levels`；空 `FreeTier` 直接进入战后商店，有效 Tier 只从 Cards 提供的同 Tier `Trait` 资格池生成三选一。数据缺失或候选不足不得跨 Tier 回退，并安全转入商店。商店始终投影 `[1级卡组, 2级卡组, 3级卡组]` 三个固定入口，按真实关次读取 `ShopTiers` 逐级启用；每个启用卡组从同 Tier 资格池确定性缓存最多三张候选，入口本身不携带具体卡牌 icon 或总价。点击后进入付费三选一，每张候选独立计价；成功购买一张才把该卡组置为“已购”，取消、余额不足或领域拒绝保持同一候选和未购状态。候选不足只显示实际 1/2 张，零张售罄，禁止跨级补位。同一 `EncounterIndex + ShopRefreshSequence` 的缓存和价格保持稳定；若页面生成后经其他效果新获得二、三级候选，则投影即时剔除该卡但不补抽、不重摇。显式整页刷新会重建全部卡组并清除本页已购状态。免费与商店均允许已拥有的 1 级卡重复叠加，并排除已拥有的 2、3 级卡。SaveVersion 16 持久化三个卡组候选和已购状态；v15 及更早的单卡页缓存显式丢弃并确定性重建。
