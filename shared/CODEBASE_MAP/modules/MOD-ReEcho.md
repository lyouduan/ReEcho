...
- 符文背包由 `UI/ReEchoRuneBackpackWidget.*` 与 `ReEchoRuneBackpackEntryWidget.*` 消费商店过滤后的只读名称/未装备份数/图标；`WBP_ReEchoRuneBackpack` 和 `WBP_ReEchoRuneBackpackEntry` 唯一拥有尺寸、边框、文字样式和真实嵌套示例。商店只计算锚点避让、填 Tooltip 并转发稳定索引与原槽 occurrence；不重建视觉树，不修改 Run 合成/装备/存档权威。回归见 `ReEchoRuneBackpackTests.cpp`，调整入口见 UI 模块文档。
...
- `UReEchoUIFlowCoordinatorSubsystem` 递归遍历注册屏幕及嵌套 Widget，为按钮统一绑定 hover/基础 confirm，并通过无玩法状态的 `UReEchoButtonVisualFeedback` 缩放按钮完整视觉根；单按钮 Overlay 即使其 Button 自带 Content 仍优先作为完整根。具体 Widget 可用稳定语义覆盖按钮反馈，普通与商店卡面点选发布 `UI.CardSelect`，后续确认/领取不重复；GameMode 只在真实关闭、拒绝、购买、卡牌揭示开始及装备集合成功变化后追加对应结果事件。购买与自动装备可依次发布 `UI.Purchase`、`UI.Equip`，未变化的重复装备不发布 `UI.Equip`；显式卸下成功发布 `UI.Unequip`。Settings/Restart 的作者ing WBP Root 是正常表现权威，原生整页构建只在没有 Root 时降级。
- `UReEchoCombatAudioAdapterComponent` 消费 Combat 最终事件；玩家攻击提交把稳定 `WeaponId` 作为 `Combat.Attack` 变体，最终命中把 `FReEchoDamageEvent.Element` 转换为稳定 ElementId 作为 `Combat.Hit` 变体，已结算元素反应把 `ReactionBehaviorId` 作为 `Combat.Reaction` 变体。Enemy/Boss Host 把已提交 Intent 翻译为其专用攻击事件，Enemy Archetype 决定 Enemy/Boss 路由，Echo Host 发布自身生命周期。时间碎片 Pickup 只在 `GrantTimeShards` 成功且关闭碰撞后发布 `Item.Pickup`，表现结束不参与入账。
...
- 数据链：主策划/音频公共工作簿继续生成 `Content/Data/*.csv`；`ReEchoDifficultyParty/Standard/Nightmare.xlsx` 各自完整拥有怪物、技能、Boss、分关数值、时间碎片掉落、Stage、Encounter、Wave 和出生配置，经 `sync_difficulty_xlsx_to_csv.py` 生成同名隔离包。`FReEchoCsvDataRegistry::LoadDifficultySnapshot` 只在新局/继续游戏按需加载并缓存，不在每关或每只怪物读盘。旧怪物/Encounter 工作簿只保留启动兼容镜像，不是新局难度权威。
...
- 怪物编译：选定难度包发布 `enemies`、`enemy_abilities`、`boss_phases`、`enemy_combat_stats`；Definition 编译以 `EnemyId + EncounterIndex` 读取本局锁定快照，按场次覆盖 MaxHealth、ContactDamage、AttackInterval，再把相对移速乘数以 `BaseMoveSpeed=210` 归一为运行时 cm/s。运行时不得回读 XLSX，也不得在 Host 复制成长或仇恨默认值。
...
- 武器表现资源由 `/Game/ReEcho/DataAsset/Weapon` 下的一武器一 `UReEchoWeaponPresentationProfile` 配置，并由 `DA_WeaponPresentationCatalog` 按 VisualKey 唯一解析；预加载器只聚合已启用的 Soft Reference。Profile 提供 Charge、Travel、DamageApplied 三个可选阶段槽，并保留明确的 AttackCommitted 槽承载长剑/镰刀提交斩击和枪口 Muzzle。
...
- 武器释放特效挂点（Plan126/137）：`AReEchoWeaponActor::WeaponAttackVfxRoot` 是 Player/Echo 释放类武器 VFX 的最终位置权威。它绑定当前可见武器组件并继承共享手部挂点、镜像后的 `HeldOffsetRatio`、最终尺寸、贴图旋转及长剑/镰刀动作；Profile 的中心化 `AttackVfxAnchorRatio` 可覆盖一般武器默认局部点。Gun 特例从其 StaticMesh 局部 Bounds 中心沿最终可见枪管方向加减实际半枪长，负 X Scale 仅用于换算本地端点，因此缩放和旋转后的左右枪口仍位于枪身端点中心。弓/枪投射物从该根的世界位置生成，Travel 随后跟随载体。枪的 `Pattern.GunShot` 提交还会把 Gun AttackCommitted 的 `NS_People_Bullet_spark` 附着该根立即播放；枪不再在目标命中位置重复播放该火花。其他 DamageApplied 仍使用命中世界位置。
- 近战范围表现适配（Plan148）：WeaponActor 在一次 Commit 上统一应用静态 AttackStep 与临时范围层，并把最终范围、基础范围、倍率和角度随 `AttackCommitted` 快照。长剑和镰刀刀光从 `WeaponAttackVfxRoot` 生成，其 Niagara Component 的 DA 声明局部径向轴消费该倍率。Weapon Presentation DA 可让持有武器本体独立消费同一最终/基础倍率：长剑仅缩放其尺寸主轴，镰刀以 `400cm` 表现基础长度在相机平面 XY 同比缩放，两者均钳制于 `0.5..2.0`。群攻成长获得/过期会刷新持有尺寸；根节点、Slot Offset、镜像和玩法命中不参与倍率重算。
- 镰刀刀光旋转（Plan148）：`Pattern.ScytheSweep` 继续使用 AttackStep 权威的 `360°` 玩法命中；`PlayerScytheSlash` 将攻击方向投影到世界 XY，以组件局部 `+Z` 对齐世界 `+Z`。专用 `NS_People_Sickle_Attack_01` 的所有启用 Emitter 使用 Local Space，Sprite Renderer 使用 `CustomFacingVector` 而非 FaceCamera，并在激活前消费 `User.GroundNormal=(0,0,1)`；粒子位置和单张 Sprite 法线都服从地面对齐组件，不通过禁用深度测试规避穿模。DA `Offset.Location.Z` 控制离地高度，`Offset.Scale` 控制基础尺寸，不改变玩法范围。
- 镰刀 FullSpin（Plan148）使用可继承相机平面旋转的透明 StaticMesh Plane，而非会在渲染阶段覆盖组件角度的 Billboard；WeaponActor 用手部挂点、当前镜像后 `HeldOffsetRatio` 与旋转四元数反向补偿 Actor Location，使镰刀可见组件中心在整圈动作中保持不动。长剑 TripleSwing 继续以手部挂点为枢轴，不共享该补偿。
...
- 卡牌事务成功并完整提交 `CurrentBuild` 后，Run 通过 `OnCardHealthCommitted` 发布最终只读 StatBlock 与类型化生命调整；授卡、商店付款/购买、击杀、受伤触发、回响死亡和关末结算都走同一出口。批量授卡把所有子结果合并，`FillToMax` 优先于 `SetToStatPoint`，后续无生命变化的子结果不得覆盖已有意图；若旧逻辑改了 `HpMax/HpPoint` 却遗漏意图，Run 以事务前后差异回退为 `SetToStatPoint`。GameMode 只把最终命令一次转交 Player Combatant。失败事务不得发布，Widget 不订阅该事件反向改生命。
...
- 商店武器/符文：`parts.csv` 的 `ShopEnabled/ShopPrice` 生成报价；三个槽以独立的 `WeaponRuneRefreshSequence` 缓存稳定 ContentId 与确定性价格，购买只改变已购/已装备状态，不得用缩小后的资格池重算其余槽。Run 为武器、符文、兼容旧商品和卡牌组统一投影 `EffectivePrice/bCanPurchase`，其中当前关【喂，打劫！】把内容物实际价降为零；UI 不得再按原价、折扣或余额自行重算购买资格。主刷新每个商店关次受 `shop_refresh_rules` 限制并显示剩余次数，只推进武器/符文页；刷新行为本身仍按规则收费，不受免费内容物影响。卡牌 `FreeShopRefresh` 是独立免费额度，优先消费且不占用关次付费上限，免费用尽后才消费表中付费次数与价格，`NoShopRefresh` 阻断两类刷新。下一关重置付费预算，未用免费额度继续由 Card Runtime/SaveGame 持有；SaveVersion 17 持久化当前关次、序列和已用付费次数，读档不得重置。Plan160：符文购买先生成完整候选事务；新购符文能与已装备同级直接合成时，在原槽升级；否则兼容当前武器且对应槽有剩余容量时，先放入候选装备再结算合成，新槽合成产物保留装备。核心仍仅一个，双重武器槽的非核心按实际容量填空；满槽/异武器符文继续入背包，不挤占已有装备。只有购买提供的AcquiredId触发填空，手动卸下和换武器不会顺带装备背包库存。`ReEchoRuneInventory::TrySettle` 先结算背包内 2 合 1，再结算背包与装备连锁，直到稳定；购买、手动替换/卸下及换武器卸下都复用 `TryResolveRuneInventory`，最终通过 WeaponRuntime 装备校验才一次提交，失败不扣费、不消耗材料。每次购买成功后 GameMode 都重取完整商店只读投影，使 `OwnedParts`、`EquippedParts`、武器、卡牌、货币和背包同步更新；`OwnedParts` 只投影与当前武器类型兼容的已拥有符文，不能因长剑与镰刀共享 `Grip` 槽而串包，`OwnedPartIds` 仍保留全部所有权。稳定缓存保证重取不会重摇未购买报价。初始武器和购买武器进入 `OwnedWeaponIds`；购买整把武器与背包换装都复用 `ReEchoWeaponRuntime::TrySelectWeapon`，只保留兼容符文，不兼容符文仍拥有但卸下。`TryEquipOwnedWeapon` 是不扣费、不刷新页面的窄事务，未知、禁用或未拥有武器不得改变构筑。诅咒银行把现金与债务分开存储并优先还债，商店/HUD 只读显示 `TimeShards - TimeShardDebt`，支付资格仍由原始现金与赊账规则裁决。SaveVersion 14 起持久化武器背包与稳定武器/符文商店页；v12 及更早存档以当前装备武器迁移最小拥有集合，v13 存档保留武器背包并重建商店页。
...
- 符文持久化（Plan160/v26）：`RuneAcquisitionCounts` 是包含装备的总份数，`OwnedPartIds` 是其去重投影，背包份数为总数减装备占用；`StartRun` 和清空全部符文的卡牌效果同时清空份数。保存当前页 `PurchasedWeaponPartOfferIds`，防止合成消耗了 I 后读档复活已购报价。旧档只从有效拥有/装备恢复最小可靠份数，并保守消费已持有族的缓存报价；不虚构副本、不重摇其他槽、不重置刷新额度。III 持有同时阻断新报价、缓存投影和购买入口。统一购买审计的 `runeBackpack` 追加真实份数。
