...
- 武器释放特效挂点（Plan126）：`AReEchoWeaponActor::WeaponAttackVfxRoot` 是 Player/Echo 释放类武器 VFX 的最终位置权威。它绑定当前可见武器组件并继承共享手部挂点、镜像后的 `HeldOffsetRatio`、最终尺寸、贴图旋转及长剑/镰刀动作；Profile 的中心化 `AttackVfxAnchorRatio` 可覆盖四武器默认局部点。CombatVfx 只消费该根节点，Travel 仍跟随载体，DamageApplied 仍使用命中世界位置。
