...
- 战斗 VFX：详细设计见 [`MOD-ReEchoVFX.md`](MOD-ReEchoVFX.md)。`UReEchoCombatVfxComponent` 装配在 Player、Echo 与 Enemy Host，只读订阅 Combat/Enemy 类型化事件；三类 Host 的 `EffectsRoot` 下分别暴露 Blueprint 可编辑的 `AttackVfxRoot` / `HurtVfxRoot`，攻击与受击位置互不耦合；`FReEchoCombatVfxCatalog` 是语义到完整 Niagara/纹理路径的唯一映射。兔子三颗可见子弹以 `(AttackIdentity, VolleyBallIndex)` 一一跟随 EnemyHost 逐球快照；表现不拥有轨迹、碰撞、伤害或存档。
