...
- 武器释放特效挂点（Plan126）：`AReEchoWeaponActor::WeaponAttackVfxRoot` 是 Player/Echo 释放类武器 VFX 的最终位置权威。它绑定当前可见武器组件并继承共享手部挂点、镜像后的 `HeldOffsetRatio`、最终尺寸、贴图旋转及长剑/镰刀动作；Profile 的中心化 `AttackVfxAnchorRatio` 可覆盖四武器默认局部点。弓/枪投射物从该根的世界位置生成，Travel 随后跟随载体；父武器平面为负 X Scale 时，局部挂点抵消该贴图镜像，保证最终屏幕侧不被二次翻转。DamageApplied 仍使用命中世界位置。
