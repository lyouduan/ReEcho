...
- `UReEchoUIFlowCoordinatorSubsystem` 为注册屏幕的按钮统一绑定 hover/基础 confirm，并通过无玩法状态的 `UReEchoButtonVisualFeedback` 缩放按钮完整视觉根；单按钮 Overlay 即使其 Button 自带 Content 仍优先作为完整根。Settings/Restart 的作者ing WBP Root 是正常表现权威，原生整页构建只在没有 Root 时降级；GameMode 仅在真实关闭、拒绝、购买、卡牌选择、卡牌揭示开始及装备集合成功变化后追加对应 UI 事件。购买与自动装备可依次发布 `UI.Purchase`、`UI.Equip`，未变化的重复装备不发布 `UI.Equip`；显式卸下成功发布 `UI.Unequip`。
