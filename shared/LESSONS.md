...
### GAME-25. 元素反应拆成纯规则，Actor 只持有目标状态与反馈 [UE]

**来源**：ReEcho Plan 14

可交换顺序、消耗附着和倍率计算应集中在无世界依赖的纯规则模块；投射物只携带来袭元素，敌人只保存当前附着并把结果交给既有伤害路径。这样玩家与回响天然共用语义，组合边界也能用轻量自动化覆盖。

### GAME-26. 周期奖励必须分开统计普通选择与奖励选择 [UE]

**来源**：ReEcho Plan 15

“每四次普通选择额外一次”不能直接用总卡牌数取模，否则奖励卡也进入计数，后续周期会提前。把普通选择计数和“当前是奖励选择”作为独立运行标记，并为第二个周期补回归测试。

### GAME-27. 范围触发与范围结算使用两个纯判定 [UE]

**来源**：ReEcho Plan 16

爆破类敌人的引信触发半径和最终伤害半径是两条不同规则：进入外圈只启动一次不可逆倒计时，倒计时结束时再用内圈判断伤害。把两个边界提成纯函数，可直接覆盖边界值并保留玩家逃离窗口。

### GAME-28. 表驱动运行时必须和静态校验同严，应用效果先候选后提交 [UE]

**来源**：ReEcho Plan 22，角色/构筑 CSV 迁移及规划审查修正。
...
**来源**：ReEcho Plan 23，元素/状态/反应 CSV 迁移。
...
### GAME-31. 连续阻塞式菜单用显式续流程标记，界面之间不要恢复游戏 [UE]
...
- 同一克隆内，上游先约定窄且稳定的只读消费面，下游即可用本地分支并行；跨机器消费者只使用已经进入 `origin/main` 的稳定面，并在集成前审计与复验。远端不保留规划或任务旁支。
