...
本文件只包含 Executor 操作。最小启动顺序遵循 `AGENTS.md`，硬约束遵循 `PROJECT_RULES.md`。Planner-Executor 模式选择由 `PROGRAMMER_RULES.md` 管理；远端协作安全检查默认适用于所有模式，跳过具名步骤需先提醒风险并取得准确人工确认。

本文件中的风险性禁止项和例外统一遵循 `shared/PROJECT_RULES.md` 的“风险操作的人类确认与执行”：先提醒风险并提出是否建议问程序，获得有权限的人对准确操作的确认后执行。

## 安全开始

1. 默认在隔离的 `plan/<id>-<short>` 分支/worktree 中工作，不直接在 `main` 中实现；人工明确要求例外时，先提醒未隔离修改和误发布风险并确认。
...
3. `Reserved` 所有权行只声明意图，不阻塞互不重叠或只读工作。默认不写入其他所有者的 `Active` `Exclusive` 资源；例外需说明覆盖/冲突风险并取得有权限的人确认。
...
Executor 默认不得推送任何远端引用、合并/发布 `main`、创建 PR/MR、移除自己的 worktree 或删除远端引用。若有权限的人明确要求其中某项风险操作，按 `PROJECT_RULES.md` 提醒准确目标和协作影响、明确建议问程序并确认后执行；这不会自动授权无关的后续操作。
