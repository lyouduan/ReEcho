...
- Startup supports new/continue, character and three-weapon initial selection, in-encounter save-and-quit, animated trait draw and post-draw shop flow.
...
- Weapon switching, parts, ordered attack steps, reactions, save/recording revisions and active-run pinned snapshots are connected for player and echo consumers.
...
|---|---|---|
| Combat/run | Six encounters, Boss gate, GAS input/effects/cooldowns, CSV weapons/parts/reactions and pinned run/echo snapshots | Combined human combat/save regression |
| Presentation | 2D actors, weather, start/continue/loadout, draw/shop, inventory and stats UI | Human visual/DPI/menu regression; Plan26 read-only weapon UI work is reserved |
...
| Validation | Canonical XLSX check, 10 Python sync tests, Editor Development build and all 34 `ReEcho.*` automation tests pass on the integrated Plan25 baseline | Human PIE/usability evidence |
