...
- The current loop includes manual player movement/attacks, sphere projectiles, four enemy archetypes, UI health bars, damage VFX and hit reactions, deterministic recording, translucent echo playback and automatic six-encounter advancement.
- UE 5.8 installed/release Editor Development builds successfully and `ReEcho.Recording.InterpolatesAndHolds` automation passes.
- Human PIE play-feel acceptance is still required; the project must not be presented as a finished vertical slice.

## Current progress

- Milestone A combat skeleton is implemented as a runtime-generated Shapes prototype.
- Player basic/power attacks and echo attacks use visible sphere projectiles; Niagara is used only after non-zero damage.
- Enemies include grunt, shield, bomber and boss behaviors plus stagger, knockback and shake feedback.
...
|---|---|---|---|
| A - Combat skeleton | Player, projectiles, four enemies, encounter clock, recording and echo | Implemented, human tuning pending | PIE readability/pacing approval + broader deterministic tests |
...
| C - Build/run | Stats, four characters/weapons, cards, shops, currency, six encounters | Graybox loop only; data scaffold not imported | Data importer + full content run |
| D - Elements/keystones | Reactions, paths, multi-echo, anchor, Boss | Partial seams only | Vertical slice acceptance |
| E - Validation | Fixed-seed regressions, tuning, A/B test | One recording test | Go/No-Go report |
...
- Automation: `scripts/ue/Run-Automation.cmd -Filter ReEcho` finds and passes the recording interpolation test.
...
- Only recording interpolation has deterministic automation; combat, projectile, round advancement and hit reactions need tests.
- Experimental `AllToolsets` can emit unrelated GameFeatureData/Niagara Python commandlet warnings.
- Human PIE checks remain necessary for movement, projectile visibility, hit feel, health-bar readability and echo clarity.
