...
本文件只包含 Planner 操作。启动顺序由 `AGENTS.md` 定义；项目硬规则和验证位于 `PROJECT_RULES.md`。Planner-Executor 模式选择由 `PROGRAMMER_RULES.md` 管理；用户采用该模式时，遵循本文件的默认流程门禁。跳过具名门禁必须先按 `PROJECT_RULES.md` 提醒风险并取得准确人工确认。
...
- **可删除时删除**：过期 worktree 的准确路径已核验、状态干净、必需变化已集成或明确保留在其他位置、没有活跃所有权/进程仍需使用，且分支满足非强制删除规则后，及时移除 worktree，再删除已合并本地分支。脏 worktree、未合并分支或强制删除只有在逐项提醒丢失风险并获得准确人工确认后才可执行。
...
8. 默认只在检查准确路径和 `git status --short` 后移除干净、已合并 worktree，并使用 `git branch -d` 删除已合并本地分支。使用 `--force`、删除脏 worktree 或强制删除未合并分支时，必须按 `PROJECT_RULES.md` 提醒准确丢失内容和恢复点并取得人工确认。
...
6. 默认只推送 `main`，不强推或推送本地规划/任务分支。若人工确认强推、删除/创建其他远端引用或发布任务分支，必须先提示 Git 历史与协作风险、明确建议问程序，并按 `PROJECT_RULES.md` 取得对准确远端引用和候选的确认。远端前进或拒绝推送时停止；范围、提交或风险变化时重建/重验候选并重新获得授权。
