...
- AI 创建提交前，仓库级 `user.name` 必须同时包含当前已确认的 GitHub 人类账号和执行该提交的 AI 身份，例如 `JosephLE910 + Codex`；`user.email` 必须使用该账号已验证或 GitHub noreply 地址。人类账号未知或未经用户确认时，必须在提交前停止并询问，不能只写 AI 名称。
