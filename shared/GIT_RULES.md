...
本门禁适用于程序路线的每一次 `origin/main` 推送，包括仅 Plan、Markdown、规则、源码、内容或混合候选。Planner-Executor 模式选择本身不能豁免；跳过构建或远端协作检查的具名步骤必须按 `shared/PROJECT_RULES.md` 提醒风险、建议问程序并取得准确人工确认。

程序路线每次推送 `origin/main` 前，必须使用项目标准 UE 5.8 安装版/发行版完整构建最新、准确的最终集成候选：
