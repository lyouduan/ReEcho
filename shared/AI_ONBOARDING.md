...
- **策划稳定身份**：用于复用个人远端分支和读取 `shared/DESIGNER_EXPERIENCE/<策划身份>.md`；不是项目 Planner 身份。
...
- 每个 AI 可自由组织本地分支、worktree、提交和并行工作；大任务仍须先发布正式 Plan。
- 项目不维护 Exchange 或远端写锁；pull/push 前 fetch 并审计他人提交的物理冲突、逻辑冲突和耦合，由用户选择集成语义。
