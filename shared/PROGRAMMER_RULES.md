...
- 轻量（非 Planner-Executor）路线直接处理请求，不执行正式 Plan/所有权/生命周期流程，但仍按任务选择 Planner 或 Executor 职责，并默认遵守下方协作与安全检查；跳过具名步骤必须先完成风险提醒和人工确认。
- 两种模式默认都不授权擅自扩张范围、覆盖未提交内容、强推、删除脏 worktree 或执行未请求的不可逆操作；这些风险操作在按 `PROJECT_RULES.md` 完成提醒并获得准确人工确认后可以执行。隐瞒冲突、伪造证据和替策划/美术作尚未决定的产品选择不属于可确认豁免项。
- 提交身份仅由 `shared/GIT_RULES.md` 管理；模式选择不改变提交标签。

## 远端协作安全检查（所有模式均适用）

无论是否采用 Planner-Executor 模式，本节都是默认安全协作核心。只有按 `PROJECT_RULES.md` 提醒准确风险、明确建议问程序并获得有权限的人确认后，才能跳过具名步骤。
...
- 模式、路线或会话本身不能自动豁免此检查；人工确认只授权被准确点名的当前操作。
