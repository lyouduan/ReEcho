...
- 若 fetch 发现当前批准基线之外的他人提交，在 pull、merge、rebase、cherry-pick 或 push 前停止；报告物理/Git 冲突、逻辑冲突和耦合，即使 Git 可以快进或干净合并也不例外。
- 用户决定采用远端、采用本地、组合适配或延期/拆分。AI 提供具体差异、权衡和建议，不替人决定行为所有权。
- 若没有新的他人提交，按适用门禁继续，无需额外确认。
