...
1. 只接管已存在于远端、包含策划确认报告与复现证据的准确 Issue 分支；程序不得借本流程自行创建新 `issue/...`、改名复用其他事项或把 `request/...` 当修复分支。先 fetch 最新 `origin/main` 和目标 Issue 分支，记录两者准确 tip，并审计 Issue 独有提交、源分支、日志、未决产品取舍和相对 main 的影响面。
