...
|---|---|---|---|---|---|---|---|
| 2026-08-11 | Plan 24 weapon/slot CSV | Gavyn-side Planner | InProgress/Rework; re-review failed | `origin/plan/24-weapons-slots-csv` at published planner tip `792b939`; implementation candidate `735967d`; local accepted base `28c21b8` | Seven weapon-domain CSVs; manifest/schema; registry/snapshot; Build/Run/Save/Recording; Player/Echo/Weapon/Projectile/Enemy runtime; validators/tests. Stable read-only consumer surface: concrete WeaponId identity, display/visual key, `InputSlot`, `StartSelectable`, `LoadoutOrder`, enabled lookup. Current blockers: Unity-Build test collision, idempotent equipment derivation/weapon-switch policy, and pinned echo snapshot. | Accepted local Plans 21–23 | Read-only UI design may target the advertised stable surface, but do not base acceptance or integration on `735967d`; wait for the next passing Plan24 tip before compiling/merging Plan26. Do not edit owned files. |
| 2026-08-11 | Plan 25 XLSX authoring and CSV generation | Gavyn-side Planner | Proposed/Calibrated; executor blocked | Planning broadcast `origin/coord/planning-broadcast-20260811`; implementation branch not started | Exactly one canonical `Design/Data/ReEchoData.xlsx`; seven familiar Chinese production Sheets plus protected export metadata; same-Sheet named Tables can generate multiple CSVs through `scripts/data/sync_xlsx_to_csv.py`, `--check`, and optional `--sheet`; generated CSV remains the only runtime package. | Plan 24 final weapon schema for weapon sheets; accepted Plans 21–23 for framework/other domains | Do not create a competing workbook/schema or hand-edit generated CSV as another truth. UI/gameplay that only consumes typed APIs is outside Plan25 and may continue. |
| 2026-08-11 | Plan 26 weapon read-only UI consumers | ReEcho teammate-side Planner | Proposed; ownership reservation acknowledged, executor waits for passing Plan24 base | Teammate broadcast `origin/coord/teammate-planning-broadcast-20260811` at `2054f71`; implementation branch planned as `plan/26-weapon-readonly-ui` | Writes only `UI/ReEchoInventoryShopWidget.*`, `UI/ReEchoStatsWidget.*` and Plan26-only UI tests; consumes stable weapon identity/display/query APIs and does not mutate registry/reader/schema. | Passing/accepted Plan24 consumer surface on the already integrated UI/data lineage | Reservation is approved. Continue design/tests that do not require Plan24 implementation, but do not add missing Registry/Reader APIs or merge from a failing WIP tip; Gavyn-side Planner will announce the exact integration base after Plan24 passes. |
...
- Plan24 candidate `735967d` is not accepted: planner static checks pass, but current Editor Development rebuild fails from a duplicate Unity-Build test helper; equipment derivation and echo snapshot issues are also documented at task tip `792b939`.
...
- After Plan24 passes, announce one exact integration ref/base for teammate Plan26; until then its UI ownership is reserved but implementation acceptance remains blocked.
