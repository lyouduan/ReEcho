...
| Plan 28 player automatic/manual attacks | Gavyn-side Planner | `InProgress` | `PendingBeforeClose` | Local `plan/28-player-attack-modes`; integrate current main before Review | `Isolated` Writes to player attack-mode state, GameMode integration, existing Esc pause widget, narrow additive range/target queries if needed and Plan28-only tests | Current local tip incorrectly remaps pause to P and must be reworked to the locked Esc/no-new-input decision. Plan26 may continue; user owns PIE and play-feel validation. |
| Plan 29 echo storage/save foundation | Gavyn-side Planner | `Ready` | `NotRequired` | Local `planning/26-31-local`; create local `plan/29-echo-storage-foundation` from its current main-integrated commit | `SharedContract` Writes to Core recording state, RunSubsystem, RunSaveGame and focused storage/save tests | May run beside Plans26/28. Publishes pending/latest/stored/selected-ID state, capability APIs and v4 migration for Plans30/31; no GameMode/UI writes. |
...
| Gavyn-side Planner | Plan28 / `plan/28-player-attack-modes` | `Isolated` | `Active` | Player attack-mode state, GameMode integration, existing `ReEchoRestartWidget.*` pause UI and Plan28-only tests | Rework the P-key divergence back to the locked Esc behavior. Additive target queries require a local scope update before expansion. |
| Gavyn-side Planner | Plan29 / `plan/29-echo-storage-foundation` | `SharedContract` | `Reserved` | Core echo state, RunSubsystem, RunSaveGame and Plan29-only storage/save tests | Disjoint from Plans26/28. Plans30/31 consume only the accepted contract. |
