...
| Plan 30 specific single/multi replay runtime | Gavyn-side / Gavyn-side | Gavyn-side Planner | `InProgress` | `PendingBeforeClose` | Integrated WIP on local `main`; stabilize through the Plan31 integration branch | RunSubsystem resolver plus GameMode zero/one/many Echo spawning/resume and focused tests | Unity Build exposed a duplicate test-helper symbol after integration. Fix and rerun automated integration validation; user owns PIE validation. |
| Plan 31 shop echo storage/selection UI | Gavyn-side / Gavyn-side | Gavyn-side Planner | `InProgress` | `PendingBeforeClose` | Integrated WIP on local `main`; continue on `plan/31-shop-echo-selection-ui` after syncing merged main | Existing InventoryShopWidget plus GameMode bridge and cheap UI/state tests | Planner review found close-gate failure handling and inventory/shop visibility issues. Executor must repair these on the integrated Plan30+31 baseline; user owns visual/interaction validation. |
