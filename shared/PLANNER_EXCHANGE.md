...
| Plan 29 echo storage/save foundation | Gavyn-side Planner | `Ready` | `NotRequired` | Planning ref `coord/gavyn-echo-storage-replay`; implementation starts from freshly fetched, human-approved `origin/main` on `plan/29-echo-storage-foundation` | `SharedContract` Writes to Core recording state, RunSubsystem, RunSaveGame and focused storage/save tests | May run beside Plans 26/28. Publishes pending/latest/stored/selected-ID state, capacity/capability APIs and v4 migration for Plans 30/31; no GameMode or UI writes. |
