...
|---|---|---|---|---|---|
| Codex | Plan 29 / `plan/29-ui-umg-migration` | `Isolated` + `Exclusive` | `Active` | Non-Plan26 UI coordination C++; `ReEchoEncounterHudWidget.*`; new `WBP_ReEchoEncounterHud` and subsequent non-overlapping WBP assets claimed per batch | Renumbered after canonical Plan 28 was fetched; do not extend `ReEchoRestartWidget.*` or GameMode until combined adaptation. Same-clone Unreal lock required for assets. |
