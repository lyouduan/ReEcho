...
| Plan 30 specific single/multi replay runtime | Gavyn-side Planner | `Ready` | `PendingBeforeClose` | Local `plan/30-specific-echo-replay-runtime`; Phase A starts from current local main with accepted Plan29, Phase B integrates accepted Plan28 | Phase A: RunSubsystem resolver + focused tests. Phase B: GameMode zero/one/many Echo spawning/resume + focused tests | Phase A may run beside Plan28 without touching its files. Phase B waits for Plan28 GameMode release. Limit 0 defaults to previous encounter; positive limit uses only selected stored GUIDs. |
...
| Gavyn-side Planner | Plan30 / `plan/30-specific-echo-replay-runtime` | `SharedContract` | `Reserved` | Phase A only: RunSubsystem replay resolver and Plan30-only resolver tests | May run beside Plan28. GameMode/Echo orchestration is not claimed until Plan28 is accepted and releases ownership. |
