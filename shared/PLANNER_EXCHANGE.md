...
|---|---|---|---|---|---|
| Codex | Plan 28 / `plan/28-ui-umg-migration` | `Isolated` + `Exclusive` | `Reserved` | UI coordination C++; `ReEchoEncounterHudWidget.*`; new `WBP_ReEchoEncounterHud` and subsequent non-Plan26 WBP assets claimed per batch | Batch A/B starts after pure-planning commit; same-clone Unreal lock required for assets. |
