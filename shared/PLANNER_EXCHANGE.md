...
- Current topology: `origin/main` is `df6e60a` and contains the remote-publication rules only on top of `a0f6a10`; accepted Plans 21–24 plus integrated teammate UI lineage remain published separately at `origin/coord/accepted-plan24-20260811`. Fetch the appropriate ref explicitly and do not treat the rules-only main update as an implementation sync.
...
- Decide when the accepted local baseline through Plan24 should be integrated onto the now rules-only `origin/main` at `df6e60a`; that implementation publication requires a new exact candidate and a new human authorization.
