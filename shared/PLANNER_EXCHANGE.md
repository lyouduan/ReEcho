...
| Plan 43 怪物逻辑与表现解耦 | Gavyn-side / Gavyn-side | Gavyn-side Planner | `Review` | `PendingBeforeClose` | 本地 `plan/43-enemy-logic-presentation-separation`（基线 `origin/main@4686e60`） | 已完成 `ReEchoEnemies`、EnemyLogic/Sense/Intent/Snapshot/Event、轻量 EnemyHost、Enemy Presentation、Roster、v7 保存兼容和边界门禁 | Development Build 与 Enemies/Combat/Weapons/AttackMode/Save 聚焦回归通过；等待用户 PIE，之后 FullRebuild、集成、推送和清理。不得修改 XLSX/CSV。 |
...
| Gavyn-side Planner | Plan43 / `plan/43-enemy-logic-presentation-separation` | `SharedContract` + `Exclusive` | `Active` | `Source/ReEchoEnemies/**`、Enemy 公共契约、`ReEchoEnemyActor.*`、新 `Presentation/Enemy/**`、GameMode 的敌人生成/Roster/保存/恢复/全灭窄切片 | `Review` 等待用户 PIE，验收前继续保留所有权；Plan40 保留通用 Animation2D/资产，Plan42 保留竞技场/地图/相机，其余 GameMode 代码只读。 |
...
- Plan43 不得以“新增模块文件互不冲突”为由同时改 Plan40 的 Enemy/Animation2D 表现路径或 Plan42 的 GameMode/场景接缝。并行仅限已切片目录；公共 Snapshot/Event 变化先由 Planner 协调。
