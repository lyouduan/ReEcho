...
| Plan 28 automatic/manual player attacks | Gavyn-side / Gavyn-side | Gavyn-side Planner | `InProgress` | `PendingBeforeClose` | Behavior reference `03423bf`; combined adaptation base `0726bd6` | Player-owned attack mode, deterministic target selection, GAS held-input reuse, P pause mapping and an attack-mode child panel under the latest Restart WBP | Remote Plan29 owns the page lifecycle. Preserve its UI Flow/Restart WBP and replace Plan28's old code-built UI with typed bindings. |
| Plans 30/31/32 echo storage, replay and intermission UI | Gavyn-side / Gavyn-side | Gavyn-side Planner | `InProgress` | `PendingBeforeClose` | Behavior references `36ca009`, `c4a1a37`, `a5ab273`; combined adaptation base `0726bd6` | Shared save/runtime contract plus typed post-trait shop context, embedded echo-management child WBP and narrow GameMode/intermission orchestration | Human accepted the cross-Planner combined adaptation: latest remote UI architecture/lifecycle wins; Gavyn-side storage/replay semantics win. Old code-built shop UI is reference-only. |
| Plan 33 audio runtime-module foundation | Gavyn-side / `Unassigned` | Gavyn-side Planner | `Ready` | `NotRequired` | Renumbered local Plan; implementation waits for the combined gameplay/UI baseline | Separate `ReEchoAudio` runtime module and one-way dependency contract | Renumbered from unpublished local Plan32 because remote main owns Plan29; no audio implementation is part of the current combined adaptation. |
| Plans 34-36 authored and integrated audio | Gavyn-side / `Unassigned` | Gavyn-side Planner | `Proposed` | `PendingBeforeClose` | Renumbered local Plans; depend on Plan33 | Catalog/settings, noncombat and combat/echo audio integration | Renumbered from unpublished local Plans33-35; implementation waits for Plan33 and the relevant combined UI/gameplay surfaces. |
...
| Gavyn-side Planner | Plans 28/30/31/32 / `integration/gavyn-umg-gameplay-20260812` | `SharedContract` + local `Exclusive` | `Active` | Echo save/runtime contracts, Player attack-mode backend, typed shop/Restart bindings and only the required WBP child-panel adaptations | Cross-Planner review accepted the ownership split. Do not restore old `AddToViewport`/code-built UI or bypass UI Manager/Flow Coordinator. Re-audit if remote main advances before publication. |
...
- Current combined adaptation is based on audited `origin/main` `0726bd6`. The two `review/gavyn-*` refs are evidence only and must never be merged wholesale; delete them after accepted integration.
