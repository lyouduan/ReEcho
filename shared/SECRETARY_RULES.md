...
- 默认禁止创建或推送 `origin/main` 之外的远端分支、强推、覆盖未提交内容、删除脏 worktree、跳过验证或扩张功能范围；按 `PROJECT_RULES.md` 完成具体风险提醒并获得准确人工确认后可以执行对应操作。
- 秘书不得在没有用户选择时决定产品取舍；人已明确选择后，秘书可协调或转交对应专业路线实施。伪造验证、隐瞒结果和泄露凭据不属于可确认豁免项。
...
- 默认仅在证明协调信息已不再有效后删除，并只移除路径已核验、干净且已合并的 worktree，使用非强制 Git 删除本地分支。其他清理必须先列出将丢失的提交/文件、恢复点和建议是否问程序，再取得人工确认。
...
5. 默认只将本地 `main` 非强制推送至 `origin/main`；其他远端或强推操作按 `PROJECT_RULES.md` 提醒风险、建议问程序并取得对准确引用的人工确认。随后 fetch 并比较本地/远端结果。
