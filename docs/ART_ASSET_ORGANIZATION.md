...
- 时间碎片的可编辑运行时 Prefab 为 `Content/ReEcho/Gameplay/Pickups/BP_TimeShardPickup`，专用透明材质实例为 `Content/ReEcho/Materials/Pickups/MI_TimeShardPickup`。美术在 Blueprint Class Defaults 调整图标高度、地面排序与阴影开关，在继承组件树调整 `VisualRoot/GroundRoot/GroundShadow` 的 Transform、缩放和阴影材质；不得回到 C++ 修改视觉常量。
