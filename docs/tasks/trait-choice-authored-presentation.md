...
- 用户后续要求发布并明确保留远端已有改动。原检查点 `6f9a73cf` 留在本地分支，正式快照 `8e4fd3df` tree 相同；获发布锁后合入准确远端 `8cde5261`，集成提交 `5e770f2d`。远端商店/PlayerHud 及我们的三份 WBP 均逐 blob 核对，数据不变。
- 最终 `-FullRebuild` 成功，预构建源码指纹 `325eb362f2c1`；相关 14 项 UI/商店/双选领取测试全部成功。新增弧形控件 getter 的最终格式收敛已包含于该次完整构建，签名与行为不变。额外两项旧 Sage cadence 测试在候选和干净远端基线均失败，未修改远端代码/测试，详见 Plan158 的诊断记录；不宣称全部功能测试通过。
- 用户只豁免已知 CSV/XLSX 漂移。额外发现的 UI 文档索引缺项按用户明确要求登记 `MOD-ReEchoUI`（非 Runtime Module），独立 workflow 已通过；无需为文档变动重复源码编译。完整构建、格式、LFS、预构建匹配与差异范围均已验证。
- 本次实际发布结果以远端 main 包含本记录的提交及精确引用核验为准；原本地工程与恢复分支保留。
