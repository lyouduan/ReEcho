...
Do not copy mandatory branch, ownership, status, validation or publication rules into this file. If a `docs/` page conflicts with `shared/` on collaboration, `shared/` wins and the stale page should be corrected. Runtime behavior is verified against source and tests.
