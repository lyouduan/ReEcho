...
- 正式 icon 源图统一归档为 `Content/SourceArt/UI/Cards/Icon/T_UI_CardIcon_{CardId}.png`，运行时导入到 `/Game/ReEcho/Textures/UI/Cards/Icon/T_UI_CardIcon_{CardId}`。Plan128 的 `G_2_18`—`G_2_36` 使用 `scripts/ue/import_plan128_secondary_card_icons.py` 批量导入；替换图标只改同名 SourceArt/Texture2D，不要在 WBP 或 C++ 新增按中文名称分支。
