...
| `WBP_ReEchoLoadoutSelection` | `StatusText`, `TitleText`, `DescriptionText`, `DescriptionTextScale`, `ConfirmButtonLabel`, `StageSwitcher`, `CharacterStagePanel`, `WeaponStagePanel`, `DescriptionPanel`, `SelectionArrow`, `CharacterRow`, `WeaponRow`, `ConfirmButton` |
...
- 选中箭头不是运行时移动的一张图，而是 8 个 Designer 实例：角色前三项为 `CharacterSelectionArrow0..2`，角色第 4 项沿用 `SelectionArrow`，武器为 `WeaponSelectionArrow0..3`。先用 `StageSwitcher.Active Widget Index` 和 `Designer Preview Index` 显示目标状态，再直接移动对应 Image；运行时只切换可见性，不会覆盖 Canvas Slot 位置。保留 `SelectionArrow` 名称是为了兼容已完成的猎手箭头微调。
