...
- 无资产路径必须保持 `ShopLogicScrollBox` 可滚动；回响管理使用独立底部 `ScaleBox` 托盘和高于商城美术的 Canvas ZOrder，不得重新塞回商品纵向列表或使用默认 ZOrder。
