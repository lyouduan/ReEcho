...
| `WBP_ReEchoLoadoutSelection` | `StatusText`, `TitleText`, `DescriptionText`, `DescriptionTextScale`, `ConfirmButtonLabel`, `CharacterStagePanel`, `WeaponStagePanel`, `DescriptionPanel`, `SelectionArrow`, `CharacterRow`, `WeaponRow`, `ConfirmButton` |
...
- 1920×1080 位置权威位于 `WBP_ReEchoLoadoutSelection > LoadoutDesignCanvas`。打开 Selection WBP 即显示完整角色页面；在 Class Defaults 的 `Loadout | Designer Preview` 中用 `Designer Preview Stage` 切换角色/武器页，用 `Designer Preview Index` 切换 `-1` 初始全亮态或 `0..3` 的选中态。`TitleText`、八个 Entry、两个 Stage Panel、键盘 Focus 回退用的 `DescriptionPanel` / `DescriptionTextScale > DescriptionText`、`SelectionArrow`、`ConfirmButton` 和 `ConfirmButtonLabel` 都应在同一 Designer 中按实际运行控件调整。鼠标悬停说明框本身才在 `WBP_ReEchoLoadoutTooltip` 中调整；`TooltipRootSizeBox.Width Override` 控制总宽，`TooltipFrame` 使用 `T_UI_Loadout_DescriptionPanel` 的九宫格 Brush 并控制边框，`TooltipSurface` 控制内边距与内底色，`TitleText` / `DescriptionText` 控制字体和换行。
- 选中箭头不是运行时移动的一张图，而是 8 个 Designer 实例：角色前三项为 `CharacterSelectionArrow0..2`，角色第 4 项沿用 `SelectionArrow`，武器为 `WeaponSelectionArrow0..3`。先用 Designer Preview Stage/Index 显示目标状态，再直接移动对应 Image；运行时只切换可见性，不会覆盖 Canvas Slot 位置。保留 `SelectionArrow` 名称是为了兼容已完成的猎手箭头微调。
