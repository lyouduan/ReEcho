...
- 设计意图：保持“Profile 决定具体表现、EnemyLogic 决定 Phase2”的边界；Presentation 只提供 Born 是否仍活跃，Host 转成类型化 Phase2 许可，唯一玩法反馈是禁止 Born 与 Transform 重叠，不延迟怪物出生或普通行为。
- 权威状态与依赖：Presentation 模块独占 Born 播放状态与逐帧脚点；ReEcho Host 只读取“Born 是否仍在播放”并写入类型化 Sense 许可；ReEchoEnemies 继续独占 Phase2 判定与状态推进。不存在 Presentation → Enemies 反向依赖，模块依赖方向不变。
...
2. 在 Enemy Presentation Host Adapter 暴露一次性 Born 请求和只读播放状态，并在 `ConfigureFromDefinition` 完成 Profile 装配后调用；Host 将 `!IsBornPlaying()` 写入类型化 Phase2 许可，EnemyLogic 仅在许可为真时评估/启动 Phase2，不得用该门禁控制出生、碰撞、受伤或普通 AI。
