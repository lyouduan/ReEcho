...
- Writes:
  ...
  - `Content/ReEcho/Materials/Pickups/M_TimeShardPickup.uasset`
...
- 明确排除：不调整商店价格、卡牌文本/数值、武器符文奖励数额、Boss 奖励、自动吸附或拾取音效；不把掉落区间硬编码到 C++/JSON/Widget；不修改原始 `策划数据源` 工作簿；不让 Echo/攻击载体自行发放基础怪物奖励。
...
- 奖励在敌人最终死亡事件上只结算一次，但只生成携带该数额的世界拾取物；玩家与拾取物发生有效 Overlap 后才通过 `GrantTimeShards` 增加余额。死亡本身不得直接修改 `TimeShards`。
...
- 权威状态与依赖：
  ...
  - `UReEchoRunSubsystem` 是余额、一次性卡牌经济规则、每 Run 随机种子和死亡掉落数额解析权威；世界拾取 Actor 只在玩家有效拾取时调用窄入账事务。
...
- 决策记录：
  ...
  9. 拾取物动画仍是 Actor 内的纯表现状态：生成后只移动 `VisualRoot` 做可调的阻尼落地弹跳，`GroundRoot/GroundShadow` 始终留在玩法地面；有效接触先立即入账并禁用碰撞，再隐藏阴影、令图标快速上升并通过专用材质参数连续淡出，动画结束后销毁 Actor。弹跳高度/时长/次数与拾取上升高度/时长全部由 `BP_TimeShardPickup` Class Defaults 调整，不回写 Run 经济状态。
- 相关文档同步范围：
...
- [x] 玩家与 Echo 击杀均触发同一基础掉落且每敌人最多一次；死亡只解析数额，不直接增加余额。
- [x] 死亡位置生成正式时间碎片拾取物；只有玩家碰撞拾取后余额增加，重复 Overlap 不重复入账。
...
3. 在 Run 增加持久化 `EnemyShardDropSeed` 与“解析一次敌人死亡掉落数额”的窄事务；先应用禁掉落/1.5 倍规则，重复键无副作用，但不直接修改余额。
4. 在 GameMode 的统一敌人装配路径绑定死亡事件，把当前 Encounter、EnemyId 和 SpawnIndex 交给 Run；得到正数后在死亡位置生成通用时间碎片拾取物。出生与读档恢复复用同一绑定，Enemy/Combat 不访问货币。
5. 将正式源 PNG 导入为可 Cook 的运行时 Texture2D，升级通用拾取 Actor 的 Billboard、碰撞和一次性拾取事务；武器符文继续复用同一 Actor。
6. 删除关末固定 15，仅保留下一关加成标记清理；确保同 Stage 残留敌人按死亡关次处理。
7. 增加数据、区间、确定性、存档迁移、幂等、类别、跨关、卡牌修正、拾取后入账和固定奖励删除回归测试。
8. 更新相关模块文档并完成关闭前具名审阅；最终组合候选执行 FullRebuild 后交由用户 PIE 验收。
...
| 聚焦自动化 | `scripts\ue\Run-Automation.cmd -Filter ReEcho.Data`、`-Filter ReEcho.Run`、`-Filter ReEcho.Run.EnemyShardDrops` | 数据解析、逐关矩阵、死亡幂等、跨关、卡牌修正与存档迁移通过 |
...
- 2026-08-24：确认现有 `TimeShards`、`GrantTimeShards`、卡牌禁掉落/下一关 1.5 倍与稳定敌人 `SpawnIndex` 可复用；最初候选选择逐敌死亡直接入账并以持久 Run seed 保证确定性，后被本轮用户决策替换为世界拾取物。
- 2026-08-24：新增 `经济系统/tblEnemyShardDrops` 与生成 CSV，删除旧敌人 `Reward` 列；Python 同步器、Schema、manifest 与中英文使用说明同步维护。
- 2026-08-24：Run 新增 v13 `EnemyShardDropSeed` 与已处理死亡键；GameMode 的出生/恢复共用装配函数订阅 Combat 最终死亡，按当前 Encounter 和 Archetype 逐只解析掉落；`CompleteEncounter` 不再固定发 15。
...
- 2026-08-24：按用户表现反馈增加两段可编辑动画。生成时 `VisualRoot` 从轻微离地高度做两次衰减弹跳，阴影保持贴地；拾取成功仍在接触瞬间入账和关闭碰撞，但延迟到图标快速上升、`Opacity` 连续降为 0 后销毁。新增专用 `M_TimeShardPickup` 主材质承载运行时透明参数，避免把淡出能力依赖在引擎 Paper2D 材质的隐式参数上。
- 2026-08-24：用户 PIE 发现弹跳后的图标下半部被地面切掉。根因是专用透明主材质恢复了默认深度测试，而 Material Billboard 中心位于 GameplayPlane，地面深度会裁掉中心以下的像素；修正为主材质忽略深度测试，仍以 `GroundSortPriority=-60` 保持透明对象之间的地面层遮挡关系。
...
- 原始表没有明确 Boss、未拾取实体跨存档/跨 Stage 的保留语义；本 Plan 锁定 Boss=0、余额只在实际拾取时增加，世界实体遵循 Actor 生命周期。若需要跨存档恢复未拾取实体或自动吸附，需要后续明确产品规则。
- 两个生产 XLSX 和 Run/GameMode 是近期共享热点；执行或发布前远端前进会使本次二进制表审计与构建证据失效。

### 人工验收结果/请求

- `Passed`：2026-08-24 用户确认当前掉落/拾取与视觉表现无问题，并亲自调整 `BP_TimeShardPickup` 尺寸后授权合入远端 main。
...
- `MOD-ReEcho.md`：已维护逐关基础敌人奖励权威、Run 随机/幂等、GameMode 死亡生成世界拾取物和拾取后入账边界。
- `MOD-ReEchoCards.md`：已维护禁掉落/下一关 1.5 倍只声明规则、Run 唯一解析掉落数额的边界。
- `MOD-ReEchoEnemies.md`：已维护 Enemy 只发布死亡事实、不持有区间/随机种子/拾取物/余额的边界。
