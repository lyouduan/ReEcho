...
- 人工验收：`PendingBeforeClose`，需要用户确认商店卡牌图标的双层浮窗位置、可读性与实际内容。
- 本地规划 / 实现基线：`origin/main@f0801c5a18a50c99e63c6ac24b01ac3a6d85f8ff`。
...
- Stable Reads:
  ...
  - `Design/Data/ReEchoData.xlsx`（本任务不改变策划数值或描述）
...
- 明确排除：不修改卡牌数值/描述、卡牌 icon、三选一页面、武器/符文 Tooltip、每次攻击或每次元素反应等瞬时事件日志；不为展示结果在 Widget 复制卡牌 ID 玩法分派。
...
- [ ] 代价丰收浮窗第二面板准确显示实际三选一代价，并在存读档后保持一致。
- [ ] 猎杀淬炼、元素虹吸在等待期显示待结算，完成目标关后显示实际 `物攻 +N` / `元攻 +N`，包括 `+0`，且数值与 StatBlock 的真实变化一致。
- [ ] 审计列出的其他随机/派生/累计稳定结果均通过统一类型化摘要展示；没有结果的普通卡不生成第二面板，UI/Run 无按中文描述分派。
- [ ] 旧存档迁移、原子失败、随机确定性、CardState/Recording/Save 恢复与既有玩法数值无回归。
- [ ] 第二面板紧贴原浮窗下方，描边、宽度、自动换行和 DPI 下可读性由用户 PIE 验收。
- [ ] 聚焦 Cards/Trait/Shop/Save 自动化、Development Editor 构建、项目校验和 `git diff --check` 通过；发布前最终集成候选 FullRebuild 与预构建检查通过。
...
- 规划期源码/生产表审计完成；尚未产生实现验证证据。

### 剩余风险

- Plan104 与本 Plan 在 Run/测试/文档/二进制存在物理集成重叠，最终发布顺序需以实际 `origin/main` 为准重新审计。
- 旧存档中已完成且已清空的历史派生收益无法可靠反推，兼容迁移只能保留可证明状态，不能伪造归因。

### 人工验收结果/请求

- `PendingBeforeClose`：实现后请在商店悬停具名卡牌，确认第二面板紧贴原浮窗下方且内容可读。
