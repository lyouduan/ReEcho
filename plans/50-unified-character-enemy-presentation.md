...
- 本地规划 / 实现基线：`origin/main@3fef6899aadaf3661485c5683f7dcd96f786acef`。
...
- Writes:
  ...
  - `Source/ReEcho/{Public,Private}/Presentation/Animation2D/**`
...
- 明确排除：不让 GameMode、EnemyActor、EnemyLogic 或动画 Controller 持有具体 Flipbook/Texture 路径；不按 `SpawnIndex`、生成顺序或 EnemyKind 随机选择外观；不让动画帧、动画通知或播放完成驱动伤害、移动、AI、死亡或存档；不重写历史提交；不静默删除当前主工作区未提交资产；不在 Unreal Editor 外修改 `.uasset/.umap`；不修改角色/怪物平衡、技能或攻击时序；不把表现拆成新的 Runtime Module。
...
- 受影响架构标识：
  - `MOD-ReEcho / AREA-Presentation`：Catalog 统一解析玩家和敌人的表现描述；Profile/FSM/Blueprint 分别拥有资源映射、状态选择和场景节点参数。
...
- 对应模块文档：维护 `MOD-ReEcho.md` 和 `MOD-ReEchoEnemies.md`，并已加入 Writes；审阅 `MOD-ReEchoCombat.md`、`MOD-ReEchoWeapons.md`，预期无需正文修改，因为表现仍是单向只读消费者。
...
- 权威状态与依赖：
  ...
  - Presentation Catalog 拥有 `PresentationId -> GameplayClass + Profile` 的硬引用映射，不拥有战斗状态。
  ...
  - 依赖保持 `Gameplay/Data -> stable FName -> ReEcho Presentation assets`；独立 Enemies/Combat/Weapons 模块不反向依赖 ReEcho 资产。
- 决策记录：
  1. 采用一个通用原生 Host + 可选表现子 Blueprint，而不是为每个身份创建 C++ 类；Catalog 选择 Class/Profile，GameMode 不扫描或 switch 具体身份。
...
- 相关文档同步范围：
  ...
  - `README.md`：保持 AREA 路由准确，若无新标识则记录已审阅无需修改。
...
- 基线分支/提交：`origin/main@3fef6899aadaf3661485c5683f7dcd96f786acef`；Plan-only 发布前和实现开始前再次 fetch 并核验。
...
1. 在干净 worktree 建立 Catalog Entry/Resolver 与 Profile 数据校验，增加纯 C++/资产自动化，先证明稳定 ID 能唯一解析。
2. 将 Enemy Definition 的 `PresentationId` 贯穿 Spawn/Restore -> EnemyActor -> EnemyPresentation；移除 SpawnIndex 外观选择和 Boss 静态特殊分支。
...
| C++ | `scripts/ue/Build-Editor.cmd -Configuration Development` | UHT/UBT 退出码为 0并刷新候选 |
...
- Plan 编写前 fetch 确认 `origin/main` 与本地批准基线一致为 `3fef689`，远端最大正式编号为 Plan49。
