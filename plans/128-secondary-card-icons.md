...
- Writes:
  ...
  - `Content/SourceArt/UI/Cards/Icon/T_UI_CardIcon_{CardId}.png`（当前 64 张启用且可投放卡牌；本轮实际变化为 19 张更新、8 张新增）
  - `Content/ReEcho/Textures/UI/Cards/Icon/T_UI_CardIcon_{CardId}.uasset`（同上）
  - `Content/SourceArt/UI/Cards/README.md`
  - `scripts/ue/import_plan128_complete_card_icons.py`
...
- Stable Reads:
  ...
  - 外部完整交付目录 `C:\Users\gavynqiu\Documents\miniGame\附件下载_【开普勒】回响数值与构筑体系\`。
- 影响模式：`Isolated`。
- 兼容承诺 / 下游操作：不改变卡牌稳定 ID、名称、Tier、启用/投放资格、抽取/刷新事务、存档或 UI 布局；只补充现有动态路径会解析到的 Texture2D。全部启用且可投放卡仍按 CardId 加载独立 icon，未来新增而未交付 icon 的卡牌继续安全回退。
- 明确排除：不修改任何卡牌启用/投放状态、效果、数值或文案；不导入同目录中的武器、符文、角色、元素等非卡牌素材；不使用 AI 重绘或重采样用户交付图；不修改 WBP 布局。

## 锁定目标

将用户最新完整交付目录作为权威来源，按 `cards.csv` 的 DisplayName 精确映射当前全部 64 张启用且可投放卡牌，以 `T_UI_CardIcon_{CardId}` 规范归档 SourceArt 并导入运行时 Texture2D。相对现有实现，37 张源图字节相同，19 张二级卡使用完整交付中的更新稿覆盖，另新增 8 张三级卡图标（`G_3_20/G_3_23/G_3_24/G_3_25/G_3_26/G_3_27/G_3_28/G_3_29`）。完成后当前 64 张可投放卡均不再进入通用 icon fallback。
...
- 决策记录：
  - 完整目录按 DisplayName 与当前 64 张启用且可投放卡牌达到 `64/64` 精确唯一匹配，不再依赖文件序号或标点修正。
  - 与本地 SourceArt 比较为 37 张字节相同、19 张更新、8 张新增；实际变化的 27 张均为 `512×512`。旧交付保留其原生 `230/460/920` 等尺寸，所有文件保持用户原始字节与 Alpha，不做重采样、补透明或颜色处理。
  ...
  - 本次对 64 张当前卡牌统一执行幂等 `replace_existing=True`；导入脚本必须校验准确对象路径、Texture2D 可加载且运行时尺寸与对应源图一致，避免静默继续 fallback。
- 相关文档同步范围：
...
- [x] 完整目录与当前启用且可投放卡牌达到 `64/64` 精确名称映射，全部以准确 CardId 命名归档，SHA-256 可追溯，没有图像重绘或重采样。
...
1. 固化 64 项 DisplayName—CardId 映射及源哈希，把 PNG 机械复制为规范 SourceArt 名称；非卡牌素材不进入仓库。
2. 新增基于 `unreal.Paths.project_dir()` 的幂等完整导入脚本，统一导入 64 个 Texture2D，逐项验证对象路径、可加载性与源图尺寸。
3. 运行卡牌资产覆盖审计与 `ReEcho.UI.TraitCard`，确认全部当前可投放卡命中独立 icon，卡牌启用/投放状态及玩法数据不变。
...
| 层级 | 命令/检查 | 预期证据 |
|---|---|---|
| 源图 | 文件名映射、SHA-256、PNG 尺寸检查 | 64/64 唯一映射；37 同、19 更新、8 新增；字节未被处理 |
| 导入/加载 | UE 5.8 执行 `scripts/ue/import_plan128_complete_card_icons.py` | 64/64 对象路径准确、Texture2D 可加载且尺寸等于对应源图 |
| 资产覆盖 | UE 内运行卡牌资产审计 | 全部 Enabled+Offerable 卡牌不再 fallback |
...
| 人工 | 在免费或商店三选一中查看本批卡牌 | 图标名称对应、清晰、无错误裁切或通用占位 |

## 执行记录

### 变化

- 第一批 19 张二级卡交付已完成本地实现；用户随后提供完整权威目录，本 Plan 继续扩展为当前全部 64 张启用且可投放卡牌的统一同步。
- 完整目录审计已确认 64/64 精确匹配，相对当前 SourceArt 为 37 张相同、19 张更新、8 张新增；已按 CardId 完成 SourceArt 同步和运行时 Texture2D 导入。为避免无意义二进制噪声，37 张字节不变源图对应的 UE 重保存结果未纳入候选，最终实际保留 27 组变化资产。
- 已用完整集导入脚本取代首批二级卡专用脚本；脚本从标准 CSV 枚举当前卡牌，验证唯一 ID/名称、PNG 文件头尺寸、对象路径和导入后 Texture2D 尺寸。
