...
3. 玩家与回响每次成功武器 Commit 记录 Source、WeaponId、Attack Sequence、构筑指纹、RawDamage、Critical、Element 和攻击段；现有 Resolver 日志可通过 Source/Weapon/Sequence 与该 Commit 一一关联。
...
- [ ] 确定性测试证明相同构筑得到相同指纹，卡牌/RuleFlags/符文容器顺序变化不改变指纹，真实内容变化会改变指纹。
- [ ] 日志格式测试覆盖卡牌叠层、符文槽位、有效属性、关键 runtime 计数、数据 revision 和空集合。
- [ ] Run 的开局、读档、遭遇开始、免费/付费/调试授卡、武器切换和符文装备成功路径均有明确 BuildTrace 触发点；失败事务不伪造提交后快照。
- [ ] 玩家与 Echo 成功 Commit 输出可与现有 Resolver 通过 Source/Weapon/Sequence 关联的指纹摘要；失败/回滚 Commit 不输出成功关联行。
- [ ] 日志代码不改变随机序列、BuildSnapshot、攻击伤害或存档；Shipping 不新增高频诊断输出。
