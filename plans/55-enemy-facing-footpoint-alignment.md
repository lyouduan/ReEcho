...
- `ReEcho.Enemies.Host` 三项测试均发现并执行，但当前 Plan52/角色二进制 WIP 删除了 Grunt/Bomber Blueprint 且演示目录暂时无法解析 `Animation.Idle`，自动化框架按已有 Error 日志判失败；日志未报告本 Plan 新增朝向/存档断言失败。本 Plan 不覆盖或恢复这些用户资产。
