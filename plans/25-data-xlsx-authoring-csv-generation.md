...
- `python -m pip install -r scripts\data\requirements.txt` installed the locked `openpyxl==3.1.5` dependency. Before install, the generator failed with a direct install hint instead of assuming a private runtime.
- `python scripts\data\sync_xlsx_to_csv.py --check` passed and reported generated bytes match production CSV.
...
- `_ExportMap` owns exactly the 16 manifest production CSV files and rejects absolute paths, `..`, subdirectories, unknown tables, unknown CSV names, duplicate table ownership, and duplicate output ownership.
- Stable Table names are locked to `tblCharacters`, `tblCharacterAliases`, `tblCards`, `tblCardEffects`, `tblElements`, `tblStatuses`, `tblReactions`, `tblWeaponTypes`, `tblWeapons`, `tblAttackSteps`, `tblSlotTypes`, `tblSlotProfiles`, `tblParts`, `tblPartEffects`, `tblRuntimeSmoke`, and `tblRuntimeSmokeEffects`.
...
- None. The initial canonical workbook generation is byte-identical to the accepted Plan24 CSV set for all 16 manifest production files.
- Source workbook values that differ from accepted production CSVs are retained only as protected ReferenceOnly/provenance material and are summarized in `Design/Data/ReEchoData.migration.md`.
- The weapon slot audit remains traceable: `parts.csv` has 78 rows, including 62 unnamed disabled rows, and the approved enabled batch remains unchanged.
