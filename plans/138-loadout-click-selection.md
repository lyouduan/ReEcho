...
- [ ] 角色阶段初始时，连续 Hover 四个角色不会设置 `SelectedCharacterId`，不会出现选择箭头或可用确认按钮。
- [ ] 点击角色 A 后 Hover 角色 B，A 的 Selected 图/箭头与最终确认对象不变；点击 B 后才切换到 B。
- [ ] 武器阶段满足同样的“Hover 不选中、点击才选中”规则。
- [ ] 鼠标 Hover 的原生 Tooltip、跟随定位和全局悬停缩放仍可用，且不出现锚定说明重复框。
- [ ] 键盘/手柄 Focus 仍能沿用现有候选预览/选择与锚定说明回退，不影响确认流程。
- [ ] 第一次确认仍只进入武器阶段；第二次确认仍只广播一次准确的 `(CharacterId, WeaponId)`。
- [ ] `ReEcho.UI.LoadoutSelection` 自动化覆盖未选择 Hover、已选择后 Hover 其他项、点击切换和两阶段确认边界并通过。
- [ ] Development Editor 构建、`python scripts/validate_project.py`、预构建一致性和 `git diff --check` 通过。
...
1. 保留 Entry Hover 广播与 Tooltip/悬停视觉能力，移除 Selection Widget 对 Hover→选择函数的绑定及无效处理函数。
