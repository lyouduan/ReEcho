...
- 明确排除：不接入没有生产 `WeaponId` 的法杖攻击，不新增武器或改武器表；不把工作簿目录中未映射的 `New*` 候选音乐自动选为正式资源；不改变 `Music.Menu -> MainMenu_v1.mp3`、`UI.CardSelect -> 选卡.wav`、`UI.Cancel/UI.Error/Revive -> 取消.wav`；不修改战斗、反应、拾取、购买、装备、卡牌或关卡推进的成功条件；不让音频完成回调驱动玩法；保留“项目音频事件清单”明确记录的用户 `Music.Death/Music.Victory/Ambience.Arena/Ambience.Rain`，但删除策划表没有授权的 `Music.Encounter/Combat.Attack/Combat.Hit` 通用旧回退；不使用 Exchange，不推送任务分支。
...
- 十一个稳定基础行保留语义但清空 `AssetPath`：`Music.Encounter`、`Combat.Attack`、`Combat.Hit`，以及 `Combat.Block`、`Combat.Kill`、`Enemy.Attack`、`Boss.Spawn`、`Boss.Attack`、`Echo.Spawn`、`Echo.Attack`、`Echo.End`。Stage、WeaponId、Element/Reaction 变体继续使用策划正式资源。
...
- 关闭前逐项填写审阅结果：
  - `MOD-ReEchoAudio.md`：已记录 52 行目录、41 个绑定行/11 个显式静默行、16 列 Schema、`StartTimeSeconds` 与资产白名单验证边界；
...
- [x] 十一个无策划授权的基础行仍是已知稳定定义但请求时安全静音；旧通用回退 SoundWave、全部 Generated 源 WAV 和生成预期均已删除，目录外资产无法通过静态/UE 白名单审计。
...
4. 第三阶段将十一项无授权基础行迁移为显式静音，删除通用旧回退、全部 Generated 源文件及生成器，并收紧目录白名单、导入、来源、资产与 Cook 审计。
...
| XLSX/CSV | `python scripts/data/sync_xlsx_to_csv.py --check`、聚焦数据测试 | 16 列 Schema、非负起播点、复合键、正式映射和十一项显式静音一致 |
...
- 十一个静音基础行均没有策划授权的具体声音；目录 AssetPath 可选且现有音频基础自动化已覆盖已定义空资源的安全 no-op。Stage、WeaponId、Element/Reaction 精确变体仍使用正式资源，不依赖通用回退。
- 正式源哈希验证通过 32/32；权威工作簿保护、数据区解锁、五个完整列验证和 16 列 CSV 字节同步均通过。配置的起播点为 `UI.Confirm=0.14`、`UI.Purchase=1.28`、`UI.CardSelect=0.38`、`Combat.Death=3.45`、反应/装备等相应非负值，后续可由策划直接改表。仓库不再含 `Design/Audio/Generated` 音频或相应生成入口。
