# Plan 31 - Gameplay - specific single and multi echo replay
...
- Depends on / Blocks: may execute in parallel with Plan28 under explicit human approval. Both may edit GameMode locally; the Planner owns semantic integration and preserves both behaviors. The completed Plan publishes runtime behavior consumed independently by Plan32's parallel implementation.
- Writes: Plan30's narrow replay-resolution implementation surface in RunSubsystem; `Source/ReEcho/Public/ReEchoGameMode.h`; `Source/ReEcho/Private/ReEchoGameMode.cpp`; additive Echo initialization/query changes only if required; focused Plan31 tests; this Plan's Execution notes.
- Stable Reads: Plan30 storage/capability APIs, immutable recordings, current Echo actor initialization/playback and encounter resume flow.
...
- Explicit exclusions: no storage/shop UI, no card/shop acquisition source, no recording/schema edits beyond Plan30 state, no Echo automatic-attack serialization, no runtime weapon switching, no XLSX/CSV or visual assets.
...
Every selected Echo actor contributes fog-of-war visibility under the same rule as a single Echo. Weather/fog consumers accept the full active Echo set instead of granting an implicit privilege to the first actor.
...
- [x] Executor performs formatting, project validation, Editor build, focused Plan31 automation and `git diff --check`, then gives the user a short PIE checklist. The user owns simultaneous-replay clarity and gameplay validation.
- [x] No generated products or machine-local paths are committed.

## Step 0 gate

- Baseline branch/commit: exact current local `main` containing accepted Plan30; record it before work. Do not consume or alter Plan28's WIP branch.
- Engine/build availability: human saves/closes Editor before C++ build; installed UE 5.8 only.
- Existing focused-test result: Plan30 storage/save and Plan28 integration checks are green on this base.
- Active exclusive ownership or shared-contract approval: the human explicitly approved concurrent GameMode work by Plans28/30. Plan31 must not edit Plan28's Player/Pause UI files and must record every GameMode integration point for the Planner. Stop if Plan30's final API differs from this consumer contract.
...
| Focused automation | Plan31 resolver plus affected save/recording tests | Latest/single/multi/resume behavior passes |
...
### Local-main integration result (authoritative, 2026-08-12)

- Plan31 and its Plan30 dependency are now integrated into local `main`; Plan28 gameplay and the final Plan32 typed shop child-panel adaptation remain outside this merge boundary.
- The merge preserves zero/one/many replay resolution, independent Echo spawn/resume, and all-active-Echo fog reveal while retaining the existing local shop selection bridge under UI Flow ownership.
- Static validation, Editor Development build, `CompileAllBlueprints` (0 errors, 0 warnings, 0 failed loads) and all 45 discovered `ReEcho` automation tests passed, including all five replay-resolver tests.
- The human owner played Plan31 and reported no current issue. Human validation is `Passed`; Plan31 remains `Review` until remote-baseline reconciliation and publication.
...
- `shared/PLANNER_EXCHANGE.md`：Plan31 状态 `InProgress`→`Review`，Active ownership `Reserved`→`Active`。
...
- `scripts/ue/Run-Automation.cmd -Filter ReEcho`：AUTOMATION_EXIT=0；整个 `ReEcho` 套件零 `Result={Fail}`。Plan31 的 5 个测试全部 `Result={Success}`；Plan30 的 EchoStorage/SaveGame/Storage 等受影响测试全部通过。
...
### Historical Planner integration issue (resolved)

- After merging Plan31 and Plan32, an integrated Unity Build failed because
