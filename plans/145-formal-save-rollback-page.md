...
- 关闭前逐项填写审阅结果：
  - `shared/CODEBASE_MAP/modules/MOD-ReEcho.md` 已更新：待实现后填写三槽权威、兼容与截图契约。
  - `shared/CODEBASE_MAP/modules/MOD-ReEchoUI.md` 已更新：待实现后填写正式面板与交互契约。
  - `Design/UI/ReEcho_UI修改指导.md` 已更新：待实现后填写可编辑控件、素材和所见即所得边界。
  - `ARCHITECTURE.md` / `README.md`：待关闭前审阅并记录结论。

## 锁定验收

- [ ] 主菜单“存档回溯”打开正式面板；布局、图层、正式底板、标题、关闭按钮和三行槽位在 1920x1080 下与成图一致，且在 WBP 设计器中可见、可调整。
- [ ] 三个槽能独立保存与读取；依次新建多个槽后，关卡、构筑、截图、时间戳均不串槽。
- [ ] 占用槽展示真实游戏截图、实际保存时间、正确关卡和卡牌数量；截图缺失/损坏时只降级为占位图，仍可读档。
- [ ] 点击占用槽立即读档；点击“新建存档”使用第一个空槽并进入既有新游戏流程；三槽全满时不显示新建行且不覆盖已有存档。
- [ ] 旧固定槽 `ReEchoRun` 能作为第 1 槽出现并成功读取，迁移失败不得删除旧文件。
- [ ] 运行内后续自动保存只写回当前活动槽；删除/重置一个新游戏槽不得影响其他槽。
- [ ] 设置、关于、退出、新游戏、键鼠/手柄焦点、悬停缩放与 UI 音效无回归。
- [ ] 覆盖槽摘要、旧槽兼容、槽隔离与失败降级的自动化测试通过；WBP 编译/审计通过。
- [ ] 功能结果有可观察证据。
- [ ] 必需自动化/构建检查通过。
- [ ] 视觉质量、可读性与真实截图由人工 PIE 验收通过后再关闭。
...
- 真实截图的捕获时机必须避开存档回溯覆盖层，并在无视口/截图失败时可安全降级。
- 旧槽兼容迁移必须可重复且不得提前删除唯一旧存档。

### 人工验收结果/请求

- `PendingBeforeClose`：实现完成后请求用户在 PIE 中验证视觉、真实截图和三槽独立性。
