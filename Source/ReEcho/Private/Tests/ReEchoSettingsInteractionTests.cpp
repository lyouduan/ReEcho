...
{
	UClass* SettingsClass = LoadClass<UReEchoSettingsWidget>(
		nullptr, TEXT("/Game/ReEcho/UI/WBP_ReEchoSettings.WBP_ReEchoSettings_C"));
	...
	UComboBoxString* ResolutionCombo = Cast<UComboBoxString>(
		SettingsWidget->GetWidgetFromName(TEXT("GraphicsResolutionComboBox")));
	UComboBoxString* OutputCombo = Cast<UComboBoxString>(
		SettingsWidget->GetWidgetFromName(TEXT("AudioOutputComboBox")));
	UComboBoxString* DisplayModeCombo = Cast<UComboBoxString>(
		SettingsWidget->GetWidgetFromName(TEXT("GraphicsDisplayModeComboBox")));
	UComboBoxString* QualityCombo = Cast<UComboBoxString>(
		SettingsWidget->GetWidgetFromName(TEXT("GraphicsQualityComboBox")));
	UComboBoxString* VSyncCombo = Cast<UComboBoxString>(
		SettingsWidget->GetWidgetFromName(TEXT("GraphicsVSyncComboBox")));
	...
	TestEqual(TEXT("Dragging the slider updates its delivered percentage label"),
		MasterPercent ? MasterPercent->GetText().ToString() : FString(), FString(TEXT("42%")));
	return true;
}
