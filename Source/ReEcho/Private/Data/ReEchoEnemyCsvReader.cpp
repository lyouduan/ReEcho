                 ...
                 TArray<FReEchoCsvIssue>& Issues)
{
	...
	for (const ReEchoCsv::FRow& CsvRow : Table.Rows)
	{
		...
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("MoveSpeedCmPerSecond"), 0.0f, 100000.0f, Row.MoveSpeedCmPerSecond, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("CollisionRadiusCm"), 0.1f, 100000.0f, Row.CollisionRadiusCm, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("CollisionHalfHeightCm"), 0.1f, 100000.0f, Row.CollisionHalfHeightCm, Issues);
		...
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("MovementStopDistanceCm"), 0.0f, 100000.0f, Row.MovementStopDistanceCm, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("HitReactionDurationSeconds"), 0.0f, 3600.0f, Row.HitReactionDurationSeconds, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("KnockbackSpeedCmPerSecond"), 0.0f, 100000.0f, Row.KnockbackSpeedCmPerSecond, Issues);
		...
		if (!ExpectedProfile)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("Archetype"), TEXT("Unsupported enemy archetype"));
		}
		else if (Row.BehaviorProfileId != *ExpectedProfile)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("BehaviorProfileId"), TEXT("Behavior profile does not match Archetype"));
		}
		if (!FReEchoCsvDataRegistry::IsBehaviorIdRegistered(Row.BehaviorProfileId))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("BehaviorProfileId"), TEXT("Unknown registered C++ behavior id"));
		}
		const bool bBossArchetype = Row.Archetype == TEXT("Boss");
		if (Row.bBoss != bBossArchetype)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("Boss"), TEXT("Boss flag must match Boss archetype"));
		}
		...
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("TriggerRadiusCm"), TEXT("Bomber radii and fuse must be positive"));
		}
		if (!bBomber && (!IsZero(Row.TriggerRadiusCm) || !IsZero(Row.DamageRadiusCm) || !IsZero(Row.FuseSeconds)))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("TriggerRadiusCm"), TEXT("Non-Bomber fields must use explicit zero"));
		}
		SeenIds.Add(Row.Id);
                   ...
                   TArray<FReEchoCsvIssue>& Issues)
{
	...
	ReEchoCsv::HasExactColumns(
	    Table,
	    {TEXT("Id"), TEXT("OwnerEnemyId"), TEXT("BehaviorId"), TEXT("SequenceOrder"), TEXT("Enabled"),
	     TEXT("Damage"), TEXT("WindupSeconds"), TEXT("ActiveSeconds"), TEXT("RecoverySeconds"),
	     TEXT("CooldownSeconds"), TEXT("MinRangeCm"), TEXT("MaxRangeCm"), TEXT("RadiusCm"), TEXT("WidthCm"),
	     TEXT("LengthCm"), TEXT("ProjectileSpeedCmPerSecond"), TEXT("TeleportOffsetCm"), TEXT("TargetingMode"),
	     TEXT("LockTiming"), TEXT("CleanseIntervalSeconds"), TEXT("ImmunitySeconds"), TEXT("SourceSheet"),
	     TEXT("SourceRow"), TEXT("Notes")},
	    Issues);

	const TSet<FName> AllowedBehaviors = {TEXT("Boss.MeleeSweep"), TEXT("Boss.Projectile"), TEXT("Boss.BlinkSlam"),
	                                      TEXT("Boss.PrayerBeam"), TEXT("Boss.ElementCleanse")};
	TSet<FName> SeenIds;
	...
	for (const ReEchoCsv::FRow& CsvRow : Table.Rows)
	{
		...
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("ProjectileSpeedCmPerSecond"), 0.0f, 100000.0f, Row.ProjectileSpeedCmPerSecond, Issues);
		...
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("CleanseIntervalSeconds"), 0.0f, 3600.0f, Row.CleanseIntervalSeconds, Issues);
		...
		if (!Owner)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("OwnerEnemyId"), TEXT("Unknown Enemies.Id reference"));
			continue;
		}
		if (!Owner->bEnabled || !Owner->bBoss)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("OwnerEnemyId"), TEXT("Abilities require an enabled boss owner"));
		}
		if (!AllowedBehaviors.Contains(Row.BehaviorId) || !FReEchoCsvDataRegistry::IsBehaviorIdRegistered(Row.BehaviorId))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("BehaviorId"), TEXT("Unknown registered C++ boss behavior id"));
		}
		if (Row.MinRangeCm > Row.MaxRangeCm)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("MinRangeCm"), TEXT("Minimum range cannot exceed maximum range"));
		}
		...
		if (bCleanse)
		{
			...
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("SequenceOrder"), TEXT("Passive cleanse must use zero"));
			}
			...
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("Damage"), TEXT("Cleanse damage and spatial fields must use explicit zero"));
			}
			if (Row.CleanseIntervalSeconds <= 0.0f || Row.ImmunitySeconds <= 0.0f)
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("CleanseIntervalSeconds"), TEXT("Cleanse interval and immunity must be positive"));
			}
		}
		else
		{
			if (Row.SequenceOrder <= 0)
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("SequenceOrder"), TEXT("Active abilities require a positive order"));
			}
			const FString OrderKey = Row.OwnerEnemyId.ToString() + TEXT("/") + FString::FromInt(Row.SequenceOrder);
			if (SeenOrders.Contains(OrderKey))
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("SequenceOrder"), TEXT("Duplicate owner and sequence order"));
			}
			SeenOrders.Add(OrderKey);
			if (Row.bEnabled)
			{
				EnabledActiveCounts.FindOrAdd(Row.OwnerEnemyId)++;
			}
			if (!IsZero(Row.CleanseIntervalSeconds) || !IsZero(Row.ImmunitySeconds))
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("CleanseIntervalSeconds"), TEXT("Active abilities must use explicit zero"));
			}
			if (Row.BehaviorId == TEXT("Boss.Projectile") && Row.ProjectileSpeedCmPerSecond <= 0.0f)
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("ProjectileSpeedCmPerSecond"), TEXT("Projectile speed must be positive"));
			}
			if (Row.BehaviorId == TEXT("Boss.BlinkSlam") && (Row.WindupSeconds <= 0.0f || Row.RadiusCm <= 0.0f))
			{
				ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("RadiusCm"), TEXT("Blink slam requires a warning and radius"));
			}
		}
		SeenIds.Add(Row.Id);
	...
	for (const TPair<FName, int32>& Pair : EnabledActiveCounts)
	{
		if (Pair.Value == 0)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, 1, TEXT("OwnerEnemyId"),
	...
	{
		Pair.Value.Abilities.Sort([](const FReEchoCsvEnemyAbilityRow& Left, const FReEchoCsvEnemyAbilityRow& Right)
		{
                    ...
                    TArray<FReEchoCsvIssue>& Issues)
{
	...
	ReEchoCsv::HasExactColumns(
	    Table,
	    {TEXT("Id"), TEXT("BossEnemyId"), TEXT("PhaseIndex"), TEXT("TriggerSeconds"), TEXT("EchoPolicy"),
	     TEXT("PhysicalAttackMultiplier"), TEXT("ElementalAttackMultiplier"), TEXT("AttackSpeedMultiplier"),
	     TEXT("MovementSpeedMultiplier"), TEXT("RefillHealthPolicy"), TEXT("Enabled"), TEXT("SourceSheet"),
	     TEXT("SourceRow"), TEXT("Notes")},
	    Issues);
	...
	for (const ReEchoCsv::FRow& CsvRow : Table.Rows)
	{
		...
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("PhysicalAttackMultiplier"), 0.0f, 100.0f, Row.PhysicalAttackMultiplier, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("ElementalAttackMultiplier"), 0.0f, 100.0f, Row.ElementalAttackMultiplier, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("AttackSpeedMultiplier"), 0.0f, 100.0f, Row.AttackSpeedMultiplier, Issues);
		ReEchoCsv::RequireFloat(Table, CsvRow, TEXT("MovementSpeedMultiplier"), 0.0f, 100.0f, Row.MovementSpeedMultiplier, Issues);
		...
		if (!Boss)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("BossEnemyId"), TEXT("Unknown Enemies.Id reference"));
			continue;
		}
		if (!Boss->bEnabled || !Boss->bBoss)
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("BossEnemyId"), TEXT("Phases require an enabled boss owner"));
		}
		const FString PhaseKey = Row.BossEnemyId.ToString() + TEXT("/") + FString::FromInt(Row.PhaseIndex);
		if (SeenPhases.Contains(PhaseKey))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("PhaseIndex"), TEXT("Duplicate boss phase index"));
		}
		if (Row.EchoPolicy != TEXT("DestroyEncounterEchoes"))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("EchoPolicy"), TEXT("Unsupported echo phase policy"));
		}
		if (Row.RefillHealthPolicy != TEXT("None"))
		{
			ReEchoCsv::AddIssue(Issues, Table.File, CsvRow.Line, TEXT("RefillHealthPolicy"), TEXT("First release must not refill health"));
		}
		SeenIds.Add(Row.Id);
	...
	for (TPair<FName, FReEchoCsvEnemyRow>& Pair : Snapshot.Enemies)
	{
		Pair.Value.BossPhases.Sort([](const FReEchoCsvBossPhaseRow& Left, const FReEchoCsvBossPhaseRow& Right)
		{
